...
import { PARENT_NAV_ITEMS, type ParentShellNavItem } from "@/lib/parent-shell/nav";
...
// where the link points. navItems defaults to the canonical PARENT_NAV_ITEMS
// (API-PD-006's own navItems[] source) so existing callers keep working
// without threading the shell-context response through everywhere.
export function ParentNav({ attentionCount, onSignOut, navItems = PARENT_NAV_ITEMS }: {
  attentionCount: number;
  onSignOut: () => void | Promise<void>;
  navItems?: readonly ParentShellNavItem[];
}) {
  ...
  return (
    <nav aria-label="Parent navigation">
      <div className="hidden items-center justify-between gap-1 border-b border-chakra-100 bg-white px-6 py-2 sm:flex">
        <div className="flex items-center gap-1">
          {navItems.map((destination) => (
            <Link key={destination.key} href={destination.href}
              ...
              {destination.label}
              {destination.showsAttentionBadge && <Badge count={attentionCount} />}
            </Link>
          ))}
        </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-10 flex items-stretch justify-around border-t border-chakra-100 bg-white sm:hidden">
        {navItems.slice(0, 3).map((destination) => (
          <Link key={destination.key} href={destination.href}
            aria-current={isActive(pathname, destination.href) ? "page" : undefined}
            ...
            <span className="relative">
              {destination.label}
              {destination.showsAttentionBadge && (
        ...
        <details className="relative flex-1">
          ...
          <div className="absolute bottom-full right-0 mb-2 w-44 rounded-lg border border-chakra-100 bg-white p-2 shadow-lg">
            {navItems.slice(3).map((destination) => (
              <Link key={destination.key} href={destination.href}
