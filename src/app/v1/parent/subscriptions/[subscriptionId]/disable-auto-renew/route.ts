...
import { disableSubscriptionAutoRenewal } from "@/lib/billing/bi002-service";
...
export async function POST(request: Request, { params }: { params: { subscriptionId: string } }) {
  const guard = await requireEndUserAuthorization(request, "parent.billing.auto_renew.disable");
  if (!guard.ok) return guard.response;
  if (!checkRateLimit(`disable-renewal:${guard.parent.session.sub}:${params.subscriptionId}`, 20, 60 * 60 * 1000)) {
