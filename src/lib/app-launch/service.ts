...
export async function dispatchAppLaunch(input: {
  ...
  const dispatchAccess = await evaluateAccessFresh({ learnerId: session.learner_id, appId: session.app_id,
...
export async function exchangeAppLaunch(input: {
  ...
  const auth = await verifyAppClientAssertion(input.clientAssertion, input.now, EXCHANGE_AUDIENCE);
  const db = resolveDbClient();
  ...
  if (existing) {
    ...
    return JSON.parse(existing.response_json) as { bootstrapAssertion: string; bootstrapExpiresAt: string;
      centralSessionExpiresAt: string; platformApiAccess: Awaited<ReturnType<typeof issueInitialAppGrant>> };
  }
  ...
  const access = await evaluateAccessFresh({ learnerId: session.learner_id, appId: session.app_id,
    environment: session.deployment_environment!, useCase: "launch_exchange", now: input.now });
  ...
  let result!: { bootstrapAssertion: string; bootstrapExpiresAt: string; centralSessionExpiresAt: string;
    platformApiAccess: Awaited<ReturnType<typeof issueInitialAppGrant>> };
  ...
  await resolveDbClient().transaction(async (db) => {
    ...
    const platformApiAccess = await issueInitialAppGrant({ learnerSessionId: session.id,
