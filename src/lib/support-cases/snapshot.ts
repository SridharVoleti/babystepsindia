...
async function composeParentSection(parentId: string): Promise<ParentSafeSection> {
  const db = resolveDbClient();
  const user = (await db.get<{ email: string; created_at: string }>(
    "select email, created_at from users where id=?", [parentId]))!;
  const profile = (await db.get<{ display_name: string | null; account_status: string }>(
    "select display_name, account_status from profiles where id=?", [parentId]))!;
  return {
...
async function composeLearnerSection(parentId: string, learnerId: string, includeAgeBand: boolean): Promise<LearnerSafeSection | undefined> {
  const ageAsOfDate = calendarDateInTimeZone(await getParentTimezone(parentId));
  const learner = (await listOwnedLearners(parentId, ageAsOfDate)).find((l) => l.id === learnerId);
  if (!learner) return undefined;
  const home = await composeLearnerHome(learnerId, "production", new Date());
  return {
...
async function composeBillingSection(subscriptionId: string): Promise<BillingSafeSection | undefined> {
...
async function composeNotificationSection(parentId: string): Promise<NotificationSafeSection[]> {
  const history = await composeParentCommunicationHistory(parentId, { limit: "5" }, new Date());
...
async function composeTechnicalIssueSection(learnerId: string): Promise<TechnicalIssueSafeSection | undefined> {
  const row = await resolveDbClient().get<{ id: string; status: string; updated_at: string }>(
...
export async function composeCaseSnapshotSections(caseId: string) {
  const row = await resolveDbClient().get<CaseRow>(
  ...
  } = { parent: await composeParentSection(row.parent_id) };
  ...
  if (needsLearner && row.learner_id) {
    sections.learner = await composeLearnerSection(row.parent_id, row.learner_id,
      row.category === "learner_access" || row.category === "progress_display");
  }
  if (row.category === "progress_display" && row.learner_id && row.app_id) {
    sections.progress = await composeProgressSection(row.learner_id, row.app_id);
  }
  if (["billing_question", "subscription_assignment", "payment_refund"].includes(row.category) && row.subscription_id) {
    sections.billing = await composeBillingSection(row.subscription_id);
  }
  if (row.category === "notification_delivery") {
    sections.notifications = await composeNotificationSection(row.parent_id);
  }
  if (row.category === "technical_issue" && row.learner_id) {
    sections.technicalIssue = await composeTechnicalIssueSection(row.learner_id);
  }
  return sections;
}
