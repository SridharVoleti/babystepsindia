...
function resolveAffectedApps(ref: LifecycleSourceReference): string[] {
  if (ref.appId) {
    const app = getDb().prepare("select 1 from app_registry where id=?").get(ref.appId);
  ...
  const subscription = db.prepare(
    "select product_id,product_version from subscriptions where id=?",
  ).get(ref.subscriptionId) as { product_id: string; product_version: number } | undefined;
  ...
  ).all(subscription.product_id, subscription.product_version) as { app_id: string }[];
  if (apps.length === 0) throw new EntitlementLifecycleError("RESOURCE_NOT_FOUND");
  return apps.map((row) => row.app_id);
}

function applySessionEffect(learnerId: string, appId: string, sessionEffect: SessionEffect, reason: string, now: Date) {
  if (sessionEffect === "cancel_starting" || sessionEffect === "preserve_to_hard_expiry") {
    cancelStartingSessionsForLearnerApp(learnerId, appId, reason, now);
  } else if (sessionEffect === "immediate_revoke") {
    revokeActiveLearnerSessionsForLearnerApp(learnerId, appId, reason, now);
  }
}
...
function applyRecordedEvent(row: EventRow, now: Date): ApplyLifecycleEventResult {
  const db = getDb();
  ...
  const affected = db.transaction((): AffectedEntitlementResult[] => {
    ...
    for (const appId of appIds) {
      ...
      ).get(row.learner_id, appId) as
        { id: string; state: string; lifecycle_version: number; revoked_before: string | null } | undefined;
      ...
      const alreadyApplied = db.prepare(
        "select 1 from entitlement_state_transitions where effective_entitlement_id=? and lifecycle_event_id=?",
      ).get(entitlement.id, row.id);
      ...
      ).run(newState, nextLifecycleVersion, row.id, revokedBefore, timestamp, entitlement.id);
      ...
      db.prepare(
        `insert into entitlement_state_transitions(id,effective_entitlement_id,lifecycle_event_id,previous_state,
         ...
         values(?,?,?,?,?,?,?,?,?, 'applied',?)`,
      ).run(randomUUID(), entitlement.id, row.id, entitlement.state, newState, row.effective_at,
        effect.sessionEffect, row.reason_category, nextLifecycleVersion, timestamp);

      applySessionEffect(row.learner_id, appId, effect.sessionEffect, `lifecycle:${row.event_type}`, now);
  ...
  db.prepare("update entitlement_lifecycle_events set status='applied',processed_at=?,updated_at=? where id=?")
  ...
  ).run(row.id, row.payload_hash, JSON.stringify(result), timestamp, timestamp);
...
export function applyLifecycleEvent(input: ApplyLifecycleEventInput): ApplyLifecycleEventResult {
  const db = getDb();
  ...
  const existing = db.prepare(
    "select * from entitlement_lifecycle_events where source=? and event_id=?",
  ).get(input.source, input.eventId) as EventRow | undefined;
  ...
  if (existing) {
    ...
    if (existing.status === "applied") {
      const receipt = db.prepare("select result_json from entitlement_transition_receipts where lifecycle_event_id=?")
        .get(existing.id) as { result_json: string | null } | undefined;
  ...
  if (identity) {
    ...
    ).get(input.source, identity.value) as { id: string; source_version: number } | undefined;
    ...
    if (latestApplied && input.sourceVersion === latestApplied.source_version) {
      db.prepare(
      ...
      ).run(randomUUID(), input.source, input.eventId, input.eventType, input.sourceVersion, input.effectiveAt,
        ...
        input.sourceReference.learnerId, JSON.stringify(resolveAffectedApps(input.sourceReference)), null,
  ...
  const appIds = resolveAffectedApps(input.sourceReference);
  ...
  db.prepare(
    `insert into entitlement_lifecycle_events(id,source,event_id,event_type,source_version,effective_at,
     ...
     values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, 'pending',?,?,?)`,
  ).run(eventId, input.source, input.eventId, input.eventType, input.sourceVersion, input.effectiveAt,
    input.sourceReference.subscriptionId ?? null, input.sourceReference.refundCaseId ?? null,
    input.sourceReference.disputeId ?? null, input.sourceReference.reassignmentCaseId ?? null,
  ...
  const inserted = db.prepare("select * from entitlement_lifecycle_events where id=?").get(eventId) as EventRow;
...
function completeLifecycleJob(principalId: string, jobType: string, key: string,
...
export function sweepDueLifecycleTransitions(principalId: string, input: {
  ...
  const replay = beginLifecycleJob(principalId, "sweep", input.runIdempotencyKey, requestHash, now);
  if (replay) return replay;
  const db = getDb();
  const rows = db.prepare(
    `select * from entitlement_lifecycle_events where status='pending' and effective_at<=? and id>?
     order by id limit ?`,
  ).all(input.dueBefore, input.cursor ?? "", input.limit + 1) as EventRow[];
  const page = rows.slice(0, input.limit);
  let processed = 0;
  let errors = 0;
  for (const row of page) {
    try { applyRecordedEvent(row, now); processed += 1; } catch { errors += 1; }
  }
...
export function reconcileEntitlementLifecycle(principalId: string, input: {
  ...
  const requestHash = hashPayload(input);
  const replay = beginLifecycleJob(principalId, "reconcile", input.runIdempotencyKey, requestHash, now);
  if (replay) return replay;
  const db = getDb();
  ...
  ).all(...params) as { id: string; provider: string; provider_event_id: string; subscription_id: string; occurred_at: string }[];
  const counts = { scanned: rows.length, restored: 0, skipped: 0, errors: 0 };
  for (const row of rows) {
    try {
      const subscription = db.prepare("select assigned_learner_id from subscriptions where id=?")
        .get(row.subscription_id) as { assigned_learner_id: string } | undefined;
      ...
      const stillValid = db.prepare(
        `select 1 from learner_app_entitlement_periods where subscription_id=? and period_start<=? and period_end>? limit 1`,
      ).get(row.subscription_id, row.occurred_at, row.occurred_at);
      if (!stillValid) {
        db.prepare("update financial_dispute_events set status='processed',processed_at=? where id=?")
      ...
      ).get(row.subscription_id) as { v: number | null };
      applyLifecycleEvent({
