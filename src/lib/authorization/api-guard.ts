...
import {AuthorizationModeError,authorizeEndUserActionAsync,deriveAuthorizationContextAsync,type AuthorizationAction} from "@/lib/authorization/modes";
...
export async function requireEndUserAuthorization(_request:Request,action:AuthorizationAction,
 ...
 try{const context=await deriveAuthorizationContextAsync({parentUserId:parent.context.session.sub,parentSessionId:parent.context.session.sid,
  deviceSessionId:parent.context.session.did,now:new Date()});
  await authorizeEndUserActionAsync(context,action,resource);return {ok:true as const,parent:parent.context,authorization:context,
