...
export async function registerAuthorizationActions(){const db=resolveDbClient();
 for(const [key,value] of Object.entries(AUTHORIZATION_ACTIONS))
  await db.run(`insert into authorization_actions
 ...
 sensitive=excluded.sensitive,version=case when authorization_actions.required_mode<>excluded.required_mode
  ...
  then authorization_actions.version+1 else authorization_actions.version end,active=1`,
   [key,value.mode,value.resource,"sensitive" in value&&value.sensitive?1:0]);}

async function activeParent(parentUserId:string){
...
export async function deriveAuthorizationContext(input:{parentUserId:string;parentSessionId?:string;deviceSessionId?:string;now:Date}){
 await activeParent(input.parentUserId);if(!input.parentSessionId||!input.deviceSessionId)throw new AuthorizationModeError("AUTHORIZATION_CONTEXT_UNAVAILABLE");
...
async function auditDenied(context:EndUserAuthorizationContext,action:AuthorizationAction,code:string,resourceId?:string){
...
export async function authorizeEndUserAction(context:EndUserAuthorizationContext,action:AuthorizationAction,resource?:{learnerId?:string;parentUserId?:string}){
 ...
 if(definition.mode==="app_service"){await auditDenied(context,action,"FORBIDDEN");throw new AuthorizationModeError("FORBIDDEN");}
 if(context.mode!==definition.mode){const code=definition.mode==="learner_mode"?"LEARNER_PROFILE_LOCKED":"PARENT_REAUTHENTICATION_REQUIRED";
  await auditDenied(context,action,code);throw new AuthorizationModeError(code);}
 if(resource?.parentUserId&&resource.parentUserId!==context.parentUserId){await auditDenied(context,action,"RESOURCE_NOT_FOUND",resource.parentUserId);
 ...
 if(resource?.learnerId){const allowed=context.mode==="learner_mode"?resource.learnerId===context.learnerId:
  ...
  if(!allowed){await auditDenied(context,action,"RESOURCE_NOT_FOUND",resource.learnerId);throw new AuthorizationModeError("RESOURCE_NOT_FOUND");}}
 return {allowed:true as const,action,mode:context.mode,learnerId:context.learnerId};
}

export async function buildAuthorizedLearnerQueryScope(context:EndUserAuthorizationContext,action:AuthorizationAction){
 await authorizeEndUserAction(context,action);return context.mode==="learner_mode"?{where:"id = ?",params:[context.learnerId!]}:
  {where:"owner_parent_id = ?",params:[context.parentUserId]};}

export async function activateLearnerMode(input:{parentUserId:string;parentSessionId:string;deviceSessionId:string;
 ...
 await activeParent(input.parentUserId);const timestamp=input.now.toISOString();
 await resolveDbClient().transaction(async (db:DbClient)=>{const receipt=await db.get<{credential_id:string}>(`select credential_id from learner_mode_unlock_receipts where id=?
  and parent_user_id=? and parent_session_id=? and device_session_id=? and learner_id=? and consumed_at is null and expires_at>?`,
  [input.verificationReceiptId,input.parentUserId,input.parentSessionId,input.deviceSessionId,input.learnerId,timestamp]);
 if(!receipt)throw new AuthorizationModeError("LEARNER_PROFILE_LOCKED");
 const selected=await db.get(`select 1 from learner_selection_contexts where parent_session_id=? and parent_user_id=?
  and selected_learner_id=? and expires_at>?`,[input.parentSessionId,input.parentUserId,input.learnerId,timestamp]);
 if(!selected)throw new AuthorizationModeError("RESOURCE_NOT_FOUND");
 const owned=await db.get("select 1 from learners where id=? and owner_parent_id=?",[input.learnerId,input.parentUserId]);
 if(!owned)throw new AuthorizationModeError("RESOURCE_NOT_FOUND");
 const consumed=(await db.run("update learner_mode_unlock_receipts set consumed_at=? where id=? and consumed_at is null",
  [timestamp,input.verificationReceiptId])).changes;
 if(consumed!==1)throw new AuthorizationModeError("LEARNER_PROFILE_LOCKED");
 await db.run(`insert into learner_unlock_contexts(parent_session_id,device_session_id,parent_user_id,learner_id,
  ...
  revoked_at=null,revocation_reason=null`,[input.parentSessionId,input.deviceSessionId,input.parentUserId,input.learnerId,
  receipt.credential_id,input.expiresAt.toISOString(),timestamp,timestamp]);
 await db.run("insert into account_events(id,parent_user_id,event_type,metadata) values(?,?,'learner_mode_activated',?)",
  [randomUUID(),input.parentUserId,JSON.stringify({learnerId:input.learnerId,deviceSessionId:input.deviceSessionId,
...
export async function revokeLearnerMode(input:{parentUserId:string;parentSessionId:string;deviceSessionId:string;
 ...
 and parent_user_id=? and status='active'`,[timestamp,timestamp,input.parentSessionId,input.deviceSessionId,input.parentUserId])).changes;
 if(changed)await resolveDbClient().run("insert into account_events(id,parent_user_id,event_type,metadata) values(?,?,'learner_mode_revoked',?)",
  [randomUUID(),input.parentUserId,JSON.stringify({deviceSessionId:input.deviceSessionId,reason:"parent_exit"})]);return changed===1;}
export async function revokeLearnerContextsByCredential(credentialId:string,now:Date){const timestamp=now.toISOString();return (await resolveDbClient().run(
 `update learner_unlock_contexts set status='revoked',revoked_at=?,revocation_reason='credential_revoked',version=version+1,
 updated_at=? where credential_id=? and status='active'`,[timestamp,timestamp,credentialId])).changes;}
export async function revokeLearnerContextsForSession(parentSessionId:string,now:Date){const timestamp=now.toISOString();return (await resolveDbClient().run(
 `update learner_unlock_contexts set status='revoked',revoked_at=?,revocation_reason='parent_logout',version=version+1,
 updated_at=? where parent_session_id=? and status='active'`,[timestamp,timestamp,parentSessionId])).changes;}
export async function revokeLearnerContextsForParent(parentUserId:string,now:Date){const timestamp=now.toISOString();return (await resolveDbClient().run(
 `update learner_unlock_contexts set status='revoked',revoked_at=?,revocation_reason='parent_session_revoked',version=version+1,
 updated_at=? where parent_user_id=? and status='active'`,[timestamp,timestamp,parentUserId])).changes;}
