...
async function deployment(db: DbClient, appId: string, deploymentId: string, releaseId: string) {
  return db.get<DeploymentRow>(
    "select * from app_deployment_launch_controls where deployment_id=? and app_id=? and release_id=?",
    [deploymentId, appId, releaseId],
  );
}
async function authorize(db: DbClient, input: { adminUserId: string; action: DeploymentAction; appId: string; deploymentId: string;
  ...
  const staff = await findStaffById(input.adminUserId);
  if (!staff || staff.status !== "active" || !roleHasCapability(await activeRoleKeys(staff.id), input.action))
  ...
  if (!(await deployment(db, input.appId, input.deploymentId, input.releaseId)))
...
export async function preflightDeploymentAuthorization(input: { adminUserId: string; action: DeploymentAction; appId: string;
  deploymentId: string; releaseId: string; reauthenticatedAt: Date; now?: Date }): Promise<DeploymentAuthorizationPreflight> {
  const now = input.now ?? new Date();
  const bundle = await authorize(resolveDbClient(), { ...input, now });
...
export async function mutateDeployment(input: { preflight: DeploymentAuthorizationPreflight; expectedVersion: number;
  idempotencyKey: string; now?: Date; startsAt?: Date }) {
  const now = input.now ?? new Date(), op = operation(input.preflight.action);
  ...
  return resolveDbClient().transaction(async (db: DbClient) => {
    const receipt = await db.get<{ request_hash: string; status: string; response_json: string | null }>(
      "select request_hash,status,response_json from deployment_mutation_requests where admin_user_id=? and idempotency_key=?",
    ...
    const activeBundle = await getActiveAuthorizationPolicyBundle();
    ...
    const bundle = await authorize(db, { adminUserId: input.preflight.adminUserId, action: input.preflight.action,
    ...
    const current = (await deployment(db, input.preflight.appId, input.preflight.deploymentId, input.preflight.releaseId))!;
    if (current.version !== input.expectedVersion) throw new DeploymentAuthorizationError("DEPLOYMENT_VERSION_CONFLICT");
    await db.run(`insert into deployment_mutation_requests(admin_user_id,idempotency_key,operation,deployment_id,
      request_hash,status,created_at) values(?,?,?,?,?,'processing',?)`,
      [input.preflight.adminUserId, input.idempotencyKey, op, current.deployment_id, requestHash, now.toISOString()]);
    ...
    if (op === "schedule" || op === "reschedule") {
      ...
      const reserved = await db.get(`select 1 from learner_sessions where app_id=? and deployment_environment=?
        and status in ('starting','active','disconnected','resumable') limit 1`, [current.app_id, current.environment]);
      const grant = await db.get("select 1 from app_session_grants where app_id=? and environment=? and status='active' limit 1",
        [current.app_id, current.environment]);
      if (reserved || grant) throw new DeploymentAuthorizationError("DEPLOYMENT_ACTIVE_SESSIONS");
      await db.run("update app_deployment_launch_controls set status='retired',version=version+1,updated_at=? where app_id=? and environment=? and status='published' and deployment_id<>?",
        [now.toISOString(), current.app_id, current.environment, current.deployment_id]);
      status = "published"; drain = null; end = null;
    }
    await db.run(`update app_deployment_launch_controls set drain_starts_at=?,deployment_window_ends_at=?,status=?,
      version=version+1,updated_at=? where deployment_id=? and app_id=? and release_id=? and version=?`,
      [drain, end, status, now.toISOString(), current.deployment_id, current.app_id, current.release_id, current.version]);
    const updated = (await deployment(db, current.app_id, current.deployment_id, current.release_id))!;
    const response = result(updated);
    await db.run(`insert into deployment_authorization_audit(id,admin_user_id,app_id,deployment_id,release_id,operation,
      policy_version,policy_digest,version_from,version_to,created_at) values(?,?,?,?,?,?,?,?,?,?,?)`,
      [randomUUID(), input.preflight.adminUserId, current.app_id, current.deployment_id, current.release_id, op,
        bundle.version, bundle.digest, current.version, updated.version, now.toISOString()]);
    await db.run(`update deployment_mutation_requests set status='completed',response_json=?,completed_at=?
      where admin_user_id=? and idempotency_key=?`, [JSON.stringify(response), now.toISOString(),
        input.preflight.adminUserId, input.idempotencyKey]);
    return response;
  });
}
