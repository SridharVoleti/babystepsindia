...
import type { DbClient, DbParam } from "@/lib/db-client/types";
...
async function logActivity(db: DbClient, operationChangeId: string, staffAccountId: string, canonicalAction: string,
  ...
  await db.run(
    ...
    [randomUUID(), operationChangeId, staffAccountId, canonicalAction, underlyingRole, resourceSafeId,
...
export async function createOperationChange(actor: StaffCaller, input: CreateOperationChangeInput, now = new Date()): Promise<OperationChangeSummary> {
  ...
  return resolveDbClient().transaction(async (db: DbClient) => {
    const existing = await db.get<{ id: string }>(
      ...
      [actor.staffAccountId, `idem:${input.idempotencyKey}`],
    );
    if (existing) {
      const row = (await db.get<ChangeRow>("select * from platform_operation_changes where id=?", [existing.id]))!;
    ...
    if (input.linkedSupportCaseId) {
      const kase = await db.get<{ status: string }>("select status from support_cases where id=?", [input.linkedSupportCaseId]);
    ...
    await db.run(
      `insert into platform_operation_changes(id,change_type,status,environment,app_id,primary_resource_type,
       ...
       values(?,?,'ready',?,?,?,?,?,?,?,1,?,?,?,?)`,
      [id, input.changeType, input.environment, input.appId ?? null, input.primaryResourceType ?? null,
    ...
    await logActivity(db, id, actor.staffAccountId, "admin.operations.change.create", "operations_administrator", id, "success", now);
    return toSummary((await db.get<ChangeRow>("select * from platform_operation_changes where id=?", [id]))!);
  });
}
...
export async function listOperationChanges(_actor: StaffCaller, filters: OperationChangeListFilters): Promise<{
  ...
  params.push(limit + 1);
  const rows = await resolveDbClient().all<ChangeRow>(
...
export async function getOperationChangeRow(operationChangeId: string): Promise<ChangeRow> {
  const row = await resolveDbClient().get<ChangeRow>("select * from platform_operation_changes where id=?", [operationChangeId]);
  if (!row) throw new OperationChangeError("OPERATION_CHANGE_NOT_FOUND");
  return row;
}

export async function getOperationChange(operationChangeId: string): Promise<OperationChangeSummary> {
  return toSummary(await getOperationChangeRow(operationChangeId));
}

export async function listOperationActivity(operationChangeId: string) {
  return resolveDbClient().all<{ id: string; staff_account_id: string; canonical_action: string;
...
export async function updateOperationChangeWorkflow(actor: StaffCaller, operationChangeId: string,
  input: UpdateOperationChangeWorkflowInput, now = new Date()): Promise<OperationChangeSummary> {
  return resolveDbClient().transaction(async (db: DbClient) => {
    const row = await getOperationChangeRow(operationChangeId);
    ...
    if (input.assignedStaffAccountId) {
      const staff = await findStaffById(input.assignedStaffAccountId);
    ...
    await db.run(
      ...
      [nextStatus, input.assignedStaffAccountId ?? null, input.scheduledFor ?? null, timestamp,
    ...
    await logActivity(db, operationChangeId, actor.staffAccountId, "admin.operations.change.workflow_update",
...
export async function requireOperationChangeForMutation(input: {
  operationChangeId: string; allowedTypes: readonly OperationChangeType[]; environment: string; appId?: string;
}): Promise<ChangeRow> {
  const row = await getOperationChangeRow(input.operationChangeId);
  if (TERMINAL_STATUSES.has(row.status)) throw new OperationChangeError("OPERATION_CHANGE_TERMINAL");
...
// terminal outcome.
export async function recordOperationOutcome(operationChangeId: string, actorStaffId: string, canonicalAction: string,
  outcome: "executing" | "succeeded" | "failed", resourceSafeId: string | null, now = new Date()): Promise<void> {
  const db = resolveDbClient();
  const timestamp = now.toISOString();
  await db.run(
    `update platform_operation_changes set status=?,version=version+1,updated_at=?,
     terminal_at=?,retention_due_at=? where id=?`,
    [outcome, timestamp, outcome === "succeeded" || outcome === "failed" ? timestamp : null,
      outcome === "succeeded" || outcome === "failed" ? retentionDueAt(now) : null, operationChangeId],
  );
  await logActivity(db, operationChangeId, actorStaffId, canonicalAction, "operations_administrator", resourceSafeId,
