...
function queueExpiryNotification(subscription: Subscription, now: Date) {
...
export function cancelStartingSessionsForSubscription(subscription: Subscription, now: Date,
  ...
  ).all(subscription.assigned_learner_id, subscription.product_id, subscription.product_version) as {
  ...
  for (const row of rows) {
    if (row.source === "standard_monthly" && row.standard_credit_batch_id) {
      ...
      ).run(now.toISOString(), row.standard_credit_batch_id);
      ...
      ).run(now.toISOString(), now.toISOString(), row.session_credit_id, row.id);
    }
    db.prepare(
    ...
    ).run(reason, now.toISOString(), now.toISOString(), row.id);
  }
  return rows.length;
}

// Shared by the bounded sweep and lazy entitlement checks. SQLite's write
...
export function expireGraceSubscriptionState(subscriptionId: string, now: Date) {
  return getDb().transaction(() => {
    ...
    ).run(now.toISOString(), now.toISOString(), subscriptionId, now.toISOString()).changes;
    if (changed !== 1) return { outcome: "race_lost" as const, subscriptionId };
    const cancelledReservations = cancelStartingSessionsForSubscription(subscription, now, "billing_grace_expired");
    getDb().prepare(
    ...
    getDb().prepare("insert into account_events(id,parent_user_id,event_type,metadata) values(?,?,?,?)")
      .run(randomUUID(), subscription.purchaser_parent_id, "subscription_grace_expired",
        JSON.stringify({ subscriptionId, graceEndsAt: subscription.grace_ends_at }));
    getDb().prepare("insert into account_events(id,parent_user_id,event_type,metadata) values(?,?,?,?)")
      .run(randomUUID(), subscription.purchaser_parent_id, "entitlement_ended_for_nonpayment",
...
export function findGraceCoverage(input: { learnerId: string; appId: string; now: Date }): GraceCoverage | null {
  ...
  ).get(input.learnerId, input.appId, nowIso) as { id: string } | undefined;
  if (due) expireGraceSubscriptionState(due.id, input.now);
  return null;
}
