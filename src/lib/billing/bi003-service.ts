...
function parentSubscription(parentId: string, subscriptionId: string) {
...
function currentGraceSubscription(parentId: string, subscriptionId: string, now: Date) {
  let subscription = parentSubscription(parentId, subscriptionId);
  if (subscription.payment_state === "past_due_grace" && subscription.grace_ends_at &&
    ...
    subscription = parentSubscription(parentId, subscriptionId);
  }
  return subscription;
}

export function getPaymentRecoveryStatus(parentId: string, subscriptionId: string, now = new Date()) {
  const subscription = currentGraceSubscription(parentId, subscriptionId, now);
  ...
  ).get(subscriptionId) as { status: string; failure_code: string | null;
    attempted_at: string; settled_at: string | null } | undefined;
...
export function createPaymentMethodUpdateSession(parentId: string, subscriptionId: string, input: {
  ...
  ).get(parentId, subscriptionId, input.idempotencyKey) as { request_hash: string; result_json: string } | undefined;
  if (existing) {
  ...
  return getDb().transaction(() => {
    ...
    getDb().prepare("insert into account_events(id,parent_user_id,event_type,metadata) values(?,?,?,?)")
      .run(randomUUID(), parentId, "payment_method_update_session_created",
...
export function queueRoutineRecoveryNotification(subscriptionId: string, now = new Date()) {
  ...
  ).run(id, subscriptionId, recoveryWindowKey(now), context, now.toISOString(), now.toISOString());
  return { queued: true, notificationId: id };
}

function beginGraceRun(principalId: string, key: string, requestHash: string, now: Date) {
  const existing = getDb().prepare(
    `select request_hash,status,result_json from billing_grace_job_runs
     where principal_id=? and run_idempotency_key=?`,
  ).get(principalId, key) as { request_hash: string; status: string; result_json: string | null } | undefined;
  if (existing) {
    if (existing.request_hash !== requestHash) throw new BillingAssignmentError("IDEMPOTENCY_KEY_REUSED");
  ...
  getDb().prepare(
    `insert into billing_grace_job_runs(principal_id,run_idempotency_key,request_hash,status,created_at)
     values(?,?,?,'running',?)`,
  ).run(principalId, key, requestHash, now.toISOString());
  return null;
}

export function runGraceExpirySweep(principalId: string, input: {
  provider?: string;
  cursor?: string;
  limit: number;
  ...
  const replay = beginGraceRun(principalId, input.runIdempotencyKey, requestHash, now);
  ...
  const rows = getDb().prepare(`select * from subscriptions where ${where} order by id limit ?`)
  ...
  for (const subscription of rows) {
    try {
      ...
      if (!adapter.stopRenewalRetries || !subscription.provider_subscription_ref) {
        getDb().prepare("update subscriptions set provider_retry_stop_state='unsupported',updated_at=? where id=?")
        ...
        try {
          ...
          getDb().prepare("update subscriptions set provider_retry_stop_state='confirmed',updated_at=? where id=?")
            .run(now.toISOString(), subscription.id);
        } catch {
          getDb().prepare("update subscriptions set provider_retry_stop_state='failed',updated_at=? where id=?")
            .run(now.toISOString(), subscription.id);
          counts.errors += 1;
        }
      }
  ...
  getDb().prepare(
    `update billing_grace_job_runs set status='completed',result_json=?,completed_at=?
     where principal_id=? and run_idempotency_key=?`,
  ).run(JSON.stringify(result), now.toISOString(), principalId, input.runIdempotencyKey);
  return result;
}
