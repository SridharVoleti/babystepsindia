...
create table if not exists support_cases (
  ...
  -- Rules 9, 28-29: bound exactly once from a valid lookup receipt, then
  -- immutable — a mis-bound case is closed as invalid and a new one opened,
  -- never edited (enforced in the service layer, not just documentation).
  ...
  invoice_id text,
  source_ref text,
  -- The lookup receipt this case was created from — lets a retried create
  -- call (same receiptId, receipt already consumed) return the same case
  -- idempotently instead of erroring, without overloading the generic
  -- support_case_mutation_requests table (which needs an existing case_id).
  created_from_receipt_id text not null,
  assigned_staff_account_id text references staff_accounts(id),
  -- Rules 63-68: set while status='escalated'; identifies the target role
  -- family a qualified staff member must hold to continue the case.
...
-- Rules 53-56, 73-74: append-only, never edited/deleted — a correction is a
-- new note referencing the prior one, enforced by exposing no update/delete
-- function or route at all.
...
-- Rules 73-78: same minimal/append-only shape as staff_audit_log — every
-- case action (create, lookup, assignment, sensitive-section view, note,
-- workflow change, escalation, resolve, close, reopen) gets exactly one row.
create table if not exists support_case_activity (
  id text primary key,
  case_id text not null references support_cases(id) on delete cascade,
  actor_staff_account_id text not null references staff_accounts(id),
  canonical_action text not null,
  -- Rule 69: the exact underlying role/capability the action was actually
  -- authorized under (e.g. a Super Admin continuing under Billing) — never
  -- inferred from the Super Admin display label.
...
-- Rules 17-27: short-lived, exact-match-only lookup receipt. A case can
-- only be created by presenting an unexpired receipt id here — never a
-- freeform identifier the caller types straight into POST /cases, which
-- would let case creation route around the exact-match/reason/rate-limit
-- gate the resolver enforces.
create table if not exists support_lookup_receipts (
  ...
  -- Rule 77: hash, never the plaintext identifier, in anything durable.
...
-- Same request_hash + idempotency_key composite-key replay pattern as
-- staff_mutation_requests/billing_mutation_requests, scoped to a case
-- instead of a target staff account.
...
-- Rules 92-97: append-only, correlates every owning-domain mutation back to
-- exactly one operation_change_id.
