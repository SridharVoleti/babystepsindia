...
export async function recordReauthReceipt(input: { staffSessionId: string; staffAccountId: string; now?: Date }) {
  ...
  await resolveDbClient().run(
    `insert into staff_reauth_receipts (id,staff_session_id,staff_account_id,reauth_at,valid_until,factors_json)
     values (?,?,?,?,?,?)`,
    [randomUUID(), input.staffSessionId, input.staffAccountId, now.toISOString(), validUntil.toISOString(),
...
export async function requireSensitiveReauth(input: {
  staffSessionId: string;
  staffAccountId: string;
  now?: Date;
}) {
  const now = input.now ?? new Date();
  const receipt = await resolveDbClient().get(
    ...
    [input.staffSessionId, input.staffAccountId, now.toISOString()],
  );
...
export async function hasLiveReauthReceipt(input: { staffSessionId: string; staffAccountId: string; now?: Date }): Promise<boolean> {
  try {
    await requireSensitiveReauth(input);
    return true;
  } catch {
    return false;
  }
