...
export function runReconciliationSweep(input: ReconciliationSweepInput): ReconciliationSweepResult {
  const db = getDb();
  const bounded = Math.max(1, Math.min(500, input.limit));
  const cursorKey = input.cursor ?? "";

  const cached = db.prepare(`select * from progress_integrity_sweep_runs
    where run_idempotency_key=? and cursor=?`).get(input.runIdempotencyKey, cursorKey) as SweepRunRow | undefined;
  if (cached) {
  ...
  const rows = db.prepare(`select learner_id, app_id, updated_at from learner_app_progress
    where (updated_at > ? or (updated_at = ? and learner_id > ?) or (updated_at = ? and learner_id = ? and app_id > ?))
    ${appFilter} order by updated_at, learner_id, app_id limit ?`).all(...params) as ProgressPageRow[];
  ...
  for (const row of page) {
    ...
    const priorRow = db.prepare(`select integrity_state from learner_app_progress_integrity where learner_id=? and app_id=?`)
      .get(row.learner_id, row.app_id) as { integrity_state: IntegrityClassification } | undefined;
  ...
  db.prepare(`insert into progress_integrity_sweep_runs(run_idempotency_key,cursor,environment,app_id,processed,
    next_cursor,incidents_opened,repairs_applied,created_at) values(?,?,?,?,?,?,?,?,?)`)
    .run(input.runIdempotencyKey, cursorKey, input.environment, input.appId ?? null, page.length, nextCursor,
