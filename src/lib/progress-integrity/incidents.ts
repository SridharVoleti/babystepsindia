...
export function getSafeIncident(incidentId: string) {
  const incident = getDb().prepare("select * from progress_integrity_incidents where id=?").get(incidentId) as IncidentRow | undefined;
...
export function getProgressIntegrityHealth(appId: string, environment?: string) {
  const db = getDb();
  const envClause = environment ? "and environment=?" : "";
  const params = environment ? [appId, environment] : [appId];
  const byStatus = db.prepare(`select status, count(*) as n from progress_integrity_incidents
    where app_id=? ${envClause} group by status`).all(...params) as Array<{ status: string; n: number }>;
  const byClassification = db.prepare(`select classification, count(*) as n from progress_integrity_incidents
    where app_id=? ${envClause} and status in ('open','investigating') group by classification`)
    .all(...params) as Array<{ classification: string; n: number }>;
  const openAges = db.prepare(`select created_at from progress_integrity_incidents
    where app_id=? ${envClause} and status in ('open','investigating')`).all(...params) as Array<{ created_at: string }>;
  const now = Date.now();
...
export function applyIncidentAction(input: ApplyIncidentActionInput): ApplyIncidentActionResult {
  ...
  const replay = db.prepare(`select * from progress_integrity_incident_actions where incident_id=? and idempotency_key=?`)
  ...
  return db.transaction(() => {
    const incident = db.prepare("select * from progress_integrity_incidents where id=?").get(input.incidentId) as IncidentRow | undefined;
    if (!incident) throw new ProgressIntegrityError("PROGRESS_INTEGRITY_INCIDENT_NOT_FOUND");
    if (incident.version !== input.expectedVersion) throw new ProgressIntegrityError("PROGRESS_INTEGRITY_INCIDENT_VERSION_CONFLICT");

    const integrityRow = db.prepare(`select integrity_state from learner_app_progress_integrity where learner_id=? and app_id=?`)
      .get(incident.learner_id, incident.app_id) as { integrity_state: IntegrityClassification } | undefined;
    ...
    if (input.action === "revalidate") {
      ...
      if (!input.receiptId) { result = "rejected"; resultCode = "RECEIPT_ID_REQUIRED"; }
        ...
        const receipt = db.prepare(`select to_schema_version from learner_progress_migration_receipts
          where id=? and learner_id=? and app_id=?`).get(input.receiptId, incident.learner_id, incident.app_id) as
          { to_schema_version: number } | undefined;
        const progressRow = db.prepare(`select schema_version from learner_app_progress where learner_id=? and app_id=?`)
          .get(incident.learner_id, incident.app_id) as { schema_version: number } | undefined;
        if (!receipt || !progressRow || receipt.to_schema_version !== progressRow.schema_version) {
          ...
          db.prepare(`update learner_app_progress set last_migration_receipt_id=? where learner_id=? and app_id=?`)
            .run(input.receiptId, incident.learner_id, incident.app_id);
      ...
      if (priorIntegrityState !== "read_only_safe") {
        ...
        db.prepare(`update learner_app_progress_integrity set legacy_policy_acknowledged=1 where learner_id=? and app_id=?`)
          .run(incident.learner_id, incident.app_id);
        const revalidated = reevaluate(incident, input.now);
        newIntegrityState = revalidated.classification;
    ...
    if (result !== "rejected" && newIncidentStatus !== incident.status) {
      db.prepare(`update progress_integrity_incidents set status=?, workflow_route=?, version=version+1, updated_at=?,
        resolved_at=? where id=?`)
        .run(newIncidentStatus, input.action === "open_disaster_recovery_case" ? "disaster_recovery" : incident.workflow_route,
          nowIso, RESOLVED_STATUSES.has(newIncidentStatus) ? nowIso : null, incident.id);
    } else if (result !== "rejected") {
      db.prepare(`update progress_integrity_incidents set attempt_count=attempt_count+1, version=version+1, updated_at=? where id=?`)
        .run(nowIso, incident.id);
    }

    db.prepare(`insert into progress_integrity_incident_actions(id,incident_id,action,actor_admin_id,reauthenticated_at,
