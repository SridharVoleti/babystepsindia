...
async function setup(email = "parent@example.com") {
  ...
  const created = createLearner(user.id, {
    displayName: "AARAV",
...
describe("LP-002 learner profile correction", () => {
  it("corrects a name without changing permanent identity and increments version once", async () => {
    ...
    const result = updateLearner(user.id, learner.id, {
  ...
  it("allows capitalization-only correction but treats cleaned equality as a no-op", async () => {
    const { user, learner } = await setup();
    const changed = updateLearner(user.id, learner.id, {
      displayName: "Aarav",
      expectedVersion: 1,
    ...
    const noOp = updateLearner(user.id, learner.id, {
      displayName: "  Aarav  ",
      expectedVersion: 2,
  ...
  it("rejects a stale version and leaves the learner unchanged", async () => {
    const { user, learner } = await setup();
    updateLearner(user.id, learner.id, {
      dateOfBirth: "1980-01-01",
      expectedVersion: 1,
      idempotencyKey: "20000000-0000-4000-8000-000000000004",
    }, "2026-08-04");

    expect(() => updateLearner(user.id, learner.id, {
      displayName: "Stale overwrite",
      expectedVersion: 1,
      idempotencyKey: "20000000-0000-4000-8000-000000000005",
    }, "2026-08-04")).toThrowError(new LearnerCreationError("LEARNER_VERSION_CONFLICT"));
    expect(getOwnedLearner(user.id, learner.id, "2026-08-04").displayName).toBe("AARAV");
  });
  ...
  it("replays an identical completed update and rejects conflicting key reuse", async () => {
    ...
    const first = updateLearner(user.id, learner.id, input, "2026-08-04");
    const replay = updateLearner(user.id, learner.id, input, "2026-08-05");
    ...
    }, "2026-08-05")).toThrowError(new LearnerCreationError("IDEMPOTENCY_KEY_REUSED"));
  });
  ...
  it("changes and removes approved avatars while rejecting unavailable avatars", async () => {
    ...
    const removed = updateLearner(user.id, learner.id, {
    ...
    }, "2026-08-04")).toThrowError(new LearnerCreationError("AVATAR_NOT_AVAILABLE"));
  });
  ...
  it("hides learner existence from unrelated parents and denies inactive owners first", async () => {
    ...
    expect(() => getOwnedLearner(other.id, learner.id, "2026-08-04")).toThrowError(
      new LearnerCreationError("LEARNER_NOT_FOUND"),
    );
    getDb().prepare("update profiles set account_status='deleted' where id=?").run(user.id);
    expect(() => getOwnedLearner(user.id, learner.id, "2026-08-04")).toThrowError(
  ...
  it("rejects another owned learner's normalized name without mutation", async () => {
    ...
    expect(getOwnedLearner(user.id, learner.id, "2026-08-04")).toMatchObject({
