...
describe("readLearnerAppSummarySnapshot", () => {
  it("returns exists:false with no summary when no progress row exists at all", () => {
    const snapshot = readLearnerAppSummarySnapshot(learnerId, appId);
    expect(snapshot).toEqual({ exists: false, summary: null, visibilityStatus: null });
  });

  it("returns a null summary when the progress row exists but never got a summary written", () => {
  ...
  it("returns the parsed summary and its visibility status when populated", () => {
  ...
  it("does not require an active or any learner_sessions row, unlike getCurrentProgress", () => {
