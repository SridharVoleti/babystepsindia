...
describe("AD-001 staff status changes (API-AD-007)", () => {
  it("suspends a staff account and bumps its authorization generation for fast revocation", async () => {
    ...
    const before = (await findStaffById(targetId))!;
    ...
    const after = (await findStaffById(targetId))!;
  ...
  it("blocks suspending/revoking the last active Platform Administrator (business rule 73)", async () => {
    ...
    await assignStaffRoles({ actorStaffId: adminId, targetStaffId: secondAdminId, roleKeys: ["platform_administrator"], reason: REASON, expectedVersion: (await findStaffById(secondAdminId))!.version, idempotencyKey: "grant-1", now });
    ...
    await changeStaffStatus({ actorStaffId: secondAdminId, targetStaffId: adminId, newStatus: "revoked", reason: REASON, expectedVersion: (await findStaffById(adminId))!.version, idempotencyKey: "key-3", now }); // allowed: second admin still holds platform_administrator, so first isn't the last one.

    // Now try to revoke the (only remaining) second admin — must be blocked.
    await expect(changeStaffStatus({ actorStaffId: adminId, targetStaffId: secondAdminId, newStatus: "revoked", reason: REASON, expectedVersion: (await findStaffById(secondAdminId))!.version, idempotencyKey: "key-4", now })).rejects.toThrow(new StaffIdentityError("LAST_PLATFORM_ADMINISTRATOR"));
  });
...
describe("AD-001 staff role assignment (API-AD-008)", () => {
  it("unions multi-role assignment and replaces the active set on a second call", async () => {
    ...
    expect((await activeRoleKeys(targetId)).sort()).toEqual(["billing_administrator", "support_agent"]);
  });
  ...
  it("blocks removing platform_administrator from the last active Platform Administrator", async () => {
    ...
    await assignStaffRoles({ actorStaffId: adminId, targetStaffId: secondAdminId, roleKeys: ["support_agent"], reason: REASON, expectedVersion: (await findStaffById(secondAdminId))!.version, idempotencyKey: "roles-4", now });
    ...
    await expect(assignStaffRoles({ actorStaffId: secondAdminId, targetStaffId: adminId, roleKeys: ["billing_administrator"], reason: REASON, expectedVersion: (await findStaffById(adminId))!.version, idempotencyKey: "roles-5", now })).rejects.toThrow(new StaffIdentityError("LAST_PLATFORM_ADMINISTRATOR"));
  });
});
