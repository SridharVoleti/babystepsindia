...
describe("AD-002 updateSupportCaseWorkflow (AT-AD-002-28/29/30/31/33/34/37/44)", () => {
  it("AT-28/29: a status update mutates only the case row, never a source-domain table or notification", () => {
    ...
    updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 1, idempotencyKey: randomUUID(), status: "waiting_parent" });
  ...
  it("rejects a stale expectedVersion with a conflict, never silently overwriting", () => {
    updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 1, idempotencyKey: randomUUID(), status: "in_progress" });
    expect(() => updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 1, idempotencyKey: randomUUID(), status: "resolved" }))
      .toThrow(SupportCaseError);
  });

  it("rejects a transition out of closed — closed is terminal for ordinary workflow", () => {
    updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 1, idempotencyKey: randomUUID(), status: "resolved" });
    updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 2, idempotencyKey: randomUUID(), status: "closed" });
    expect(() => updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 3, idempotencyKey: randomUUID(), status: "open" }))
      .toThrow(SupportCaseError);
  });

  it("AT-30/31: escalation preserves the same case_id and never itself grants the target role", () => {
    const before = updateSupportCaseWorkflow(staff, caseId,
  ...
  it("AT-33: a Super Admin can continue an escalated case under any of the three target roles because it explicitly holds each", () => {
  ...
  it("AT-34: an escalated action's activity row records the underlying role actually used, not the Super Admin label", () => {
    const superAdmin = seedStaffSession(["support_agent", "billing_administrator", "operations_administrator", "platform_administrator"]);
    updateSupportCaseWorkflow(superAdmin, caseId,
      { expectedVersion: 1, idempotencyKey: randomUUID(), status: "escalated", escalationRole: "billing_administrator" });
  ...
  it("AT-44: an urgent priority never bypasses a missing capability — priority is operational only", () => {
    updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 1, idempotencyKey: randomUUID(), priority: "urgent" });
    // Priority alone changes nothing about who can act on the case.
    expect(roleHasCapability(staff.roleKeys, "admin.billing.subscription.reassign")).toBe(false);
  });

  it("rejects assigning a case to a nonexistent/inactive staff account", () => {
    expect(() => updateSupportCaseWorkflow(staff, caseId,
      { expectedVersion: 1, idempotencyKey: randomUUID(), assignedStaffAccountId: randomUUID() })).toThrow(SupportCaseError);
  });
});

describe("AD-002 reopenSupportCase (AT-AD-002-38/45/46)", () => {
  it("AT-38: reopens a resolved case with a reason, bumping its version and auditing the action", () => {
    updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 1, idempotencyKey: randomUUID(), status: "resolved" });
    const reopened = reopenSupportCase(staff, caseId, { reason: "Parent called back with a follow-up question.", idempotencyKey: randomUUID() });
  ...
  it("rejects reopening a case that was never resolved", () => {
    expect(() => reopenSupportCase(staff, caseId, { reason: "Trying to reopen an open case.", idempotencyKey: randomUUID() }))
      .toThrow(SupportCaseError);
  });

  it("AT-45/46: rejects reopening a resolved case past the 24-month retention window", () => {
    updateSupportCaseWorkflow(staff, caseId, { expectedVersion: 1, idempotencyKey: randomUUID(), status: "resolved" });
    const farFuture = new Date();
    farFuture.setMonth(farFuture.getMonth() + 25);
    expect(() => reopenSupportCase(staff, caseId, { reason: "Way past retention window now.", idempotencyKey: randomUUID() }, farFuture))
      .toThrow(SupportCaseError);
  });
});
