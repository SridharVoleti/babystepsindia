...
async function fixture() {
  ...
  const bundle = await createAuthorizationPolicyBundle({ version: "1.0.0", sourceCommitSha: "a".repeat(40), now,
  ...
  await activateAuthorizationPolicyBundle({ version: bundle.version, activatedBy: user.id, now });
  return { user, app };
}
...
describe("AU-001 deployment-window authorization", () => {
  it("schedules an exact app/release with a 60-minute drain and 45-minute window under the locked policy", async () => {
    ...
    const preflight = await preflightDeploymentAuthorization({ adminUserId: user.id, action: "deployment.schedule",
      appId: app.id, deploymentId: "dep-1", releaseId: "release-1", reauthenticatedAt: now, now });
    const result = await mutateDeployment({ preflight, expectedVersion: 1, idempotencyKey: "schedule-1", now,
  ...
  it("rejects stale reauthentication and missing deployment permission", async () => {
    const { user, app } = await fixture();
    await expect(preflightDeploymentAuthorization({ adminUserId: user.id, action: "deployment.schedule", appId: app.id,
      deploymentId: "dep-1", releaseId: "release-1", reauthenticatedAt: new Date(now.getTime() - 301_000), now }))
      .rejects.toEqual(new DeploymentAuthorizationError("RECENT_REAUTHENTICATION_REQUIRED"));
    ...
    await expect(preflightDeploymentAuthorization({ adminUserId: user.id, action: "deployment.schedule", appId: app.id,
      ...
      .rejects.toEqual(new DeploymentAuthorizationError("FORBIDDEN"));
  });

  it("fails when the active policy changes between preflight and the locked mutation", async () => {
    const { user, app } = await fixture();
    const preflight = await preflightDeploymentAuthorization({ adminUserId: user.id, action: "deployment.cancel",
      appId: app.id, deploymentId: "dep-1", releaseId: "release-1", reauthenticatedAt: now, now });
    const denied = await createAuthorizationPolicyBundle({ version: "1.0.1", sourceCommitSha: "b".repeat(40), now,
      rules: [{ actionKey: "deployment.cancel", effect: "deny", principalType: "administrator", resourceType: "deployment" }] });
    await activateAuthorizationPolicyBundle({ version: denied.version, activatedBy: user.id, now });
    await expect(mutateDeployment({ preflight, expectedVersion: 1, idempotencyKey: "cancel-1", now }))
      .rejects.toEqual(new DeploymentAuthorizationError("AUTHORIZATION_POLICY_CHANGED"));
  ...
  it("rejects foreign app/release bindings and stale row versions without mutation", async () => {
    ...
    await expect(preflightDeploymentAuthorization({ adminUserId: user.id, action: "deployment.schedule", appId: app.id,
      ...
      .rejects.toEqual(new DeploymentAuthorizationError("DEPLOYMENT_NOT_FOUND"));
    const preflight = await preflightDeploymentAuthorization({ adminUserId: user.id, action: "deployment.schedule", appId: app.id,
      deploymentId: "dep-1", releaseId: "release-1", reauthenticatedAt: now, now });
    await expect(mutateDeployment({ preflight, expectedVersion: 99, idempotencyKey: "stale", now,
      startsAt: new Date("2026-08-06T12:00:00Z") })).rejects.toEqual(new DeploymentAuthorizationError("DEPLOYMENT_VERSION_CONFLICT"));
  });
  ...
  it("replays identical requests and rejects an idempotency key reused for another operation", async () => {
    ...
    expect(await mutateDeployment(input)).toEqual(await mutateDeployment(input));
    await expect(mutateDeployment({ ...input, startsAt: new Date("2026-08-07T12:00:00Z") }))
      .rejects.toEqual(new DeploymentAuthorizationError("IDEMPOTENCY_KEY_REUSED"));
  });
});
