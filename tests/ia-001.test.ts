...
describe("IA-001", () => {
  ...
  it("recovers one profile idempotently", async () => { const rows=new Map<string,any>(); const store:ParentProfileStore={find:async id=>rows.get(id)??null,insert:async id=>{const p={id,account_status:"active" as const,onboarding_status:"profile_pending" as const,auth_revoked_before:null};rows.set(id,p);return p;}}; expect((await ensureParentProfile(store,"u")).created).toBe(true);expect((await ensureParentProfile(store,"u")).created).toBe(false);expect(rows.size).toBe(1); });
