...
describe("getSafeIncident", () => {
  it("returns safe fields and allowed actions for an open incident", () => {
    const { incident } = makeConflictIncident("safe-view");
    const safe = getSafeIncident(incident.id);
  ...
  it("throws NOT_FOUND for a missing incident", () => {
    expect(() => getSafeIncident("nonexistent")).toThrow(EntitlementIntegrityError);
  });
});

describe("applyIncidentAction", () => {
  it("rejects a stale expectedVersion", () => {
    const { incident } = makeConflictIncident("stale-version");
    expect(() => applyIncidentAction({ incidentId: incident.id, action: "resolve_false_positive",
  ...
  it("is idempotent on repeated identical calls and rejects a reused key with a different action", () => {
    const { incident } = makeConflictIncident("idempotent-action");
    const first = applyIncidentAction({ incidentId: incident.id, action: "resolve_false_positive",
      actorAdminId: adminId, expectedVersion: incident.version, idempotencyKey: "act-2",
      reasonCategory: "known_false_positive", now: new Date("2026-08-21T00:00:00.000Z") });
    const replay = applyIncidentAction({ incidentId: incident.id, action: "resolve_false_positive",
      actorAdminId: adminId, expectedVersion: incident.version, idempotencyKey: "act-2",
      reasonCategory: "known_false_positive", now: new Date("2026-08-21T00:00:01.000Z") });
    expect(replay).toEqual(first);
    expect(() => applyIncidentAction({ incidentId: incident.id, action: "retry",
      actorAdminId: adminId, expectedVersion: incident.version, idempotencyKey: "act-2",
      now: new Date("2026-08-21T00:00:02.000Z") })).toThrow(EntitlementIntegrityError);
  });

  it("resolve_false_positive requires a reasonCategory", () => {
    const { incident } = makeConflictIncident("missing-reason");
    const result = applyIncidentAction({ incidentId: incident.id, action: "resolve_false_positive",
  ...
  it("resolve_false_positive closes the incident when given a reason", () => {
    const { incident } = makeConflictIncident("resolve-fp");
    const result = applyIncidentAction({ incidentId: incident.id, action: "resolve_false_positive",
  ...
  it("open_refund_case fails closed without a real refund_cases id", () => {
    const { incident } = makeConflictIncident("refund-missing");
    const result = applyIncidentAction({ incidentId: incident.id, action: "open_refund_case",
  ...
  it("open_refund_case fails closed for a refundCaseId that doesn't exist", () => {
    const { incident } = makeConflictIncident("refund-not-found");
    const result = applyIncidentAction({ incidentId: incident.id, action: "open_refund_case",
  ...
  it("open_refund_case routes to an existing refund case", () => {
    const { incident, subscriptionId } = makeConflictIncident("refund-routed");
  ...
  it("retry resolves the incident once the underlying mismatch is fixed", () => {
    const { incident, billingPeriodId } = makeConflictIncident("retry-resolves");
    getDb().prepare("update entitlement_cycles set product_version=1 where paid_cycle_id=?").run(billingPeriodId);

    const result = applyIncidentAction({ incidentId: incident.id, action: "retry",
  ...
  it("retry stays open when the underlying mismatch is still present", () => {
    const { incident } = makeConflictIncident("retry-still-conflicting");
    const result = applyIncidentAction({ incidentId: incident.id, action: "retry",
