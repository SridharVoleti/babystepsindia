...
const criteria: Criterion[] = [
  ...
  { id: 19, title: "RLS and repository scope are enabled", verify: () => {
    expect(Object.keys(supabaseTableAccess)).toHaveLength(165);
  ...
  { id: 44, title: "Deployment admin actions use exact actions reauthentication and locked validation", verify: () => { expect(Object.keys(DEPLOYMENT_ADMIN_AUTHORIZATION)).toEqual(["deployment.schedule", "deployment.reschedule", "deployment.cancel", "deployment.promote", "deployment.rollback"]); expect(Object.values(DEPLOYMENT_ADMIN_AUTHORIZATION).every((rule) => rule.recentReauthenticationSeconds && rule.transactionalReauthorization && rule.exactAppReleaseBinding)).toBe(true); has(sources.deploymentService(), /resolveDbClient\(\)\.transaction\(/); has(sources.deploymentService(), /AUTHORIZATION_POLICY_CHANGED/); has(sources.routeActions(), /deployment\.schedule/); } },
