...
describe("listPastApps — membership", () => {
  it("returns nothing for a learner with no entitlement history", () => {
    expect(listPastApps(parentId, learnerId, now)).toEqual([]);
  });

  it("excludes an app the learner still has active access to, even alongside an older ended period (rule 34)", () => {
    seedApp("app-both");
  ...
  it("includes an ended app exactly once (rule 32-33)", () => {
  ...
  it("throws RESOURCE_NOT_FOUND for a learner not owned by this parent", async () => {
    ...
    expect(() => listPastApps(other.user.id, learnerId, now)).toThrow(LearnerHomeError);
  });
});

describe("listPastApps — preserved summary", () => {
  it("shows the last safe summary when present", () => {
    endedApp("app-sum");
  ...
  it("shows a neutral unavailable reason with no invented values when no safe summary exists (rule 56)", () => {
    endedApp("app-nosum");
    const past = listPastApps(parentId, learnerId, now);
...
describe("listPastApps — subscribe again eligibility", () => {
  it("offers subscribe-again when a current single product includes the app and there is no overlap", () => {
    endedApp("app-resub");
  ...
  it("marks not_currently_sold when no active product currently includes the app", () => {
    endedApp("app-unsold");
    const past = listPastApps(parentId, learnerId, now);
    expect(past[0].subscribeAgain).toEqual({ offered: false, reason: "not_currently_sold" });
  });

  it("fails closed with multiple_current_products when more than one current product includes the app", () => {
    endedApp("app-dual");
  ...
  it("marks overlap when the learner already has current access via a different product covering the same app", () => {
...
describe("listPastApps — side effects", () => {
  it("writes nothing (rule: idempotent, side-effect-free reads)", () => {
    ...
    listPastApps(parentId, learnerId, now);
