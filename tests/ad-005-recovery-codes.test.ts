...
describe("AD-005 bootstrapRecoveryCodes (rule 51)", () => {
  it("openDb()'s own bootstrap already seeded at least two codes without recursing/hanging", () => {
    const status = getRecoveryCodeStatus();
    expect(status.activeCount).toBeGreaterThanOrEqual(2);
  });

  it("is a no-op once codes already exist — never issues a second free batch", () => {
    const before = getRecoveryCodeStatus();
    const result = bootstrapRecoveryCodes(getDb());
    expect(result).toHaveLength(0);
    expect(getRecoveryCodeStatus().activeCount).toBe(before.activeCount);
  });
});

describe("AD-005 rotateRecoveryCodes (AT-AD-005-27/28)", () => {
  it("AT-27: invalidates every previously active code", () => {
    const before = getDb().prepare("select id from platform_recovery_codes where status='active'").all() as { id: string }[];
    rotateRecoveryCodes(staffId);
  ...
  it("AT-28: new codes are shown once as plaintext here and never derivable from storage afterward", () => {
    const rotated = rotateRecoveryCodes(staffId);
  ...
  it("bumps the generation and a used/revoked code from the old generation can never be reused", () => {
    const first = rotateRecoveryCodes(staffId);
    const oldCode = first.codes[0]!;
    const second = rotateRecoveryCodes(staffId);
    expect(second.generation).toBe(first.generation + 1);
    expect(consumeRecoveryCode(oldCode, staffId, new Date())).toBe(false);
  });
});

describe("AD-005 consumeRecoveryCode (rules 55-56, 58, 64)", () => {
  it("a valid unused code is consumed exactly once — a second attempt with the same code fails", () => {
    const rotated = rotateRecoveryCodes(staffId);
    const code = rotated.codes[0]!;
    expect(consumeRecoveryCode(code, staffId, new Date())).toBe(true);
    expect(consumeRecoveryCode(code, staffId, new Date())).toBe(false);
  });

  it("an unrecognized code is rejected", () => {
    expect(consumeRecoveryCode("NOT-A-REAL-CODE-AT-ALL", staffId, new Date())).toBe(false);
  });
});
