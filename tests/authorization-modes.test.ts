...
describe("AU-002 parent and nested learner authorization modes",()=>{
 ...
 it("activates only the server-selected owned learner after trusted passkey verification",async()=>{const {user,first,second}=await fixture();
  await expect(activate({parentUserId:user.id,parentSessionId,deviceSessionId:deviceId,learnerId:first.id,
   ...
   .rejects.toThrowError(new AuthorizationModeError("LEARNER_PROFILE_LOCKED"));
  const context=await activate({parentUserId:user.id,parentSessionId,deviceSessionId:deviceId,learnerId:first.id,
 ...
 it("isolates modes by device and scopes before list pagination",async()=>{const {user,first}=await fixture();
  const learner=await activate({parentUserId:user.id,parentSessionId,deviceSessionId:deviceId,learnerId:first.id,
   credentialId:"cred-1",expiresAt:new Date("2026-08-05T11:00:00Z"),now});
  const other=await deriveAuthorizationContext({parentUserId:user.id,parentSessionId,deviceSessionId:"device-2",now});
  expect(other.mode).toBe("parent_management");
  expect(await buildAuthorizedLearnerQueryScope(learner,"learner.home.read")).toEqual({where:"id = ?",params:[first.id]});
  expect(await buildAuthorizedLearnerQueryScope(other,"parent.learners.list")).toEqual({where:"owner_parent_id = ?",params:[user.id]});});
 ...
 it("repeats authorization under the write lock and rolls back a denied mutation",async()=>{const {user,first}=await fixture();
  const preflight=await activate({parentUserId:user.id,parentSessionId,deviceSessionId:deviceId,learnerId:first.id,
   credentialId:"cred-1",expiresAt:new Date("2026-08-05T11:00:00Z"),now});
 ...
 describe("PD4-G01 authoritative modeGeneration",()=>{
  ...
  it("increments modeGeneration again on re-entering learner_mode after a prior exit",async()=>{const {user,first}=await fixture();
   await activate({parentUserId:user.id,parentSessionId,deviceSessionId:deviceId,learnerId:first.id,
    credentialId:"cred-1",expiresAt:new Date("2026-08-05T11:00:00Z"),now});
   await revokeLearnerMode({parentUserId:user.id,parentSessionId,deviceSessionId:deviceId,parentPasswordReauthenticated:true,now});
   const reentered=await activate({parentUserId:user.id,parentSessionId,deviceSessionId:deviceId,learnerId:first.id,
    credentialId:"cred-1",expiresAt:new Date("2026-08-05T11:00:00Z"),now});
