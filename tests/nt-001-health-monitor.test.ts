...
async function enqueueAt(sourceEventKey: string, createdAt: string) {
...
describe("NT-001 (NT1-G08) monitorNotificationDeliveryHealth", () => {
  ...
  it("creates a queue-age alert once the threshold is breached, and never a second one while still open", async () => {
    await enqueueAt(`evt-${randomUUID()}`, "2026-08-13T00:00:00.000Z");
  ...
  it("creates a provider-health-degraded alert on a burst of recent temporary provider failures", async () => {
    for (let i = 0; i < 5; i++) await enqueueAt(`evt-degraded-${i}`, "2026-08-13T00:00:00.000Z");
    const now = new Date("2026-08-13T00:00:00.000Z");
    await runNotificationDeliverySweep({ provider: { send: () => ({ status: "failed" as const }) }, now, limit: 20 });
