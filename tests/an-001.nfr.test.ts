...
describe("AN-001 non-functional and operational requirements", () => {
  it("keeps synchronous buffer-update p95 below 500 ms", async () => {
  ...
  it("completes a representative V1 daily run inside 30 minutes and restarts safely", async () => {
    ...
    const second = await runDailyAggregation("2026-08-05", new Date("2026-08-05T18:46:00.000Z"));
  ...
  it("declares a single-date lock, control verification, and post-success purge order", async () => {
  ...
  it("has no raw-event growth path and supports every required cohort filter", async () => {
  ...
  it("uses UTC instants internally and explicit Asia/Kolkata activity-date derivation", async () => {
  ...
  it("caps the scheduled job below 30 minutes and runs an independent 00:50 monitor", async () => {
  ...
  it("documents a reversible down path for every AN-001 migration without source restoration", async () => {
    const migrations = [
...
describe("AN-001 missed, failed, and overdue monitoring", () => {
  ...
  it("does not alert before the independent monitor window", async () => {
    expect(await monitorDailyAnalytics(new Date("2026-08-05T19:14:00.000Z")))
  ...
  it("alerts once when the expected run is missing", async () => {
    expect(await monitorDailyAnalytics(monitorAt)).toMatchObject({ activityDate: "2026-08-05", state: "missed", alertCreated: true });
    expect(await monitorDailyAnalytics(monitorAt)).toMatchObject({ state: "missed", alertCreated: false });
  ...
  it("alerts failed runs idempotently", async () => {
    ...
    expect(await monitorDailyAnalytics(monitorAt)).toMatchObject({ state: "failed", alertCreated: true });
    expect(await monitorDailyAnalytics(monitorAt)).toMatchObject({ state: "failed", alertCreated: false });
  });

  it("alerts running or late-completed runs that exceed 30 minutes", async () => {
    ...
    expect(await monitorDailyAnalytics(monitorAt)).toMatchObject({ state: "overdue", alertCreated: true });
    ...
    expect(await monitorDailyAnalytics(monitorAt)).toMatchObject({ state: "overdue", alertCreated: false });
  });

  it("reports a within-budget completed run as healthy", async () => {
    ...
    expect(await monitorDailyAnalytics(monitorAt)).toEqual({ activityDate: "2026-08-05", state: "healthy", alertCreated: false });
  });
});
