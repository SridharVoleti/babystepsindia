...
describe("BI-003 grace entry and controlled access", () => {
  it("AT-BI-003-01..05 enters exactly 168h grace at the paid boundary without a period or renewed event", async () => {
    const subscriptionId = await activate();
    const beforePeriods = (getDb().prepare("select count(*) n from billing_periods").get() as any).n;
    const result = await enterGrace(subscriptionId);
  ...
  it("AT-BI-003-04 intentional cancellation and a pre-boundary failure do not start grace", async () => {
    const cancelled = await activate("cancelled");
    const row = getDb().prepare("select version,current_period_end from subscriptions where id=?").get(cancelled) as any;
    await disableSubscriptionAutoRenewal(parentId, cancelled, { expectedVersion: row.version,
      idempotencyKey: "cancel-before-failure" }, { adapter: provider });
    await processVerifiedPaymentEvent(renewalEvent(cancelled, "cancelled", "renewal_failed"), new Date(row.current_period_end));
    ...
    const early = await activate("early");
    await processVerifiedPaymentEvent(renewalEvent(early, "early", "renewal_failed", "2026-09-09T10:00:00.000Z"),
  ...
  it("AT-BI-003-06..09 exposes restricted grace access using the old period and creates no allocation", async () => {
    const subscriptionId = await activate(); await enterGrace(subscriptionId);
    const batches = (getDb().prepare("select count(*) n from learner_app_standard_credit_batches").get() as any).n;
    const access = await evaluateAccessFresh({ learnerId, appId: APP_ID, environment: "test",
  ...
  it("AT-BI-003-11/12/14/38 lazily blocks new access at cutoff and preserves progress", async () => {
    const subscriptionId = await activate(); await enterGrace(subscriptionId);
...
describe("BI-003 retry, recovery and parent recovery UX", () => {
  it("AT-BI-003-15/16/33 stores each provider retry once and never extends grace", async () => {
    const subscriptionId = await activate(); await enterGrace(subscriptionId, "first");
    const deadline = (getDb().prepare("select grace_ends_at from subscriptions where id=?").get(subscriptionId) as any)
      .grace_ends_at;
    await processVerifiedPaymentEvent(renewalEvent(subscriptionId, "retry", "retry_failed", "2026-09-12T10:00:00.000Z"),
  ...
  it("AT-BI-003-17..21/40 timely recovery creates one anchored period, clears grace and preserves identity", async () => {
    const subscriptionId = await activate(); await enterGrace(subscriptionId);
    const before = getDb().prepare("select * from subscriptions where id=?").get(subscriptionId) as any;
    const result = await processVerifiedPaymentEvent(renewalEvent(subscriptionId, "recovered", "payment_recovered",
  ...
  it("AT-BI-003-26 reconciliation applies a missing timely recovery through the same event transition", async () => {
    const subscriptionId = await activate(); await enterGrace(subscriptionId);
    ...
    const result = await reconcileBilling("billing-reconcile", { provider: "contract-provider", environment: "test",
...
describe("BI-003 cutoff, ordering and boundaries", () => {
  ...
  it("AT-BI-003-30/32 allows delayed delivery of timely settlement after expiry and has one final paid period", async () => {
    ...
    await processVerifiedPaymentEvent(renewalEvent(subscriptionId, "delayed", "delayed_settlement",
  ...
  it("AT-BI-003-31 rejects a payment settled after the deadline without silent reactivation", async () => {
    ...
    await runGraceExpirySweep("billing-recovery", { limit: 100, runIdempotencyKey: "late-expire" },
      { now: new Date("2026-09-17T10:00:01.000Z"), adapters: { "contract-provider": provider } });
    await expect(processVerifiedPaymentEvent(renewalEvent(subscriptionId, "late", "payment_recovered",
      "2026-09-17T10:00:01.000Z"), new Date("2026-09-17T10:00:02.000Z"))).rejects.toThrow(new BillingAssignmentError("PAYMENT_RECOVERY_WINDOW_EXPIRED"));
    expect((getDb().prepare("select payment_state from subscriptions where id=?").get(subscriptionId) as any).payment_state)
      .toBe("inactive_nonpayment");
  });

  it("AT-BI-003-34 cancellation during grace keeps the exact existing deadline", async () => {
    const subscriptionId = await activate(); await enterGrace(subscriptionId);
    const row = getDb().prepare("select version,grace_ends_at from subscriptions where id=?").get(subscriptionId) as any;
    await disableSubscriptionAutoRenewal(parentId, subscriptionId, { expectedVersion: row.version,
  ...
  it("AT-BI-003-39 rolls back recovery when its atomic event write fails", async () => {
    const subscriptionId = await activate(); await enterGrace(subscriptionId);
    getDb().exec("drop table account_events");
    await expect(processVerifiedPaymentEvent(renewalEvent(subscriptionId, "atomic", "payment_recovered",
      "2026-09-14T10:00:00.000Z"), new Date("2026-09-14T10:00:01.000Z"))).rejects.toThrow();
    expect((getDb().prepare("select count(*) n from billing_periods where subscription_id=?")
