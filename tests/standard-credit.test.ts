...
async function fund(now: Date) {
  return await fundStandardSession({ learnerId, appId, timezone: tz, now });
}

describe("SC-002 standard monthly credit batches", () => {
  it("allocates eight credits effective at local month start, expiring end of the following local month", async () => {
    const batch = await ensureMonthlyStandardAllocation(learnerId, appId, tz, new Date("2026-08-15T09:00:00.000Z"));
  ...
  it("is idempotent — repeat/concurrent calls return exactly one batch (AC6/AC8)", async () => {
    const now = new Date("2026-08-15T09:00:00.000Z");
    const first = await ensureMonthlyStandardAllocation(learnerId, appId, tz, now);
    const second = await ensureMonthlyStandardAllocation(learnerId, appId, tz, new Date("2026-08-20T09:00:00.000Z"));
    expect(second).toEqual(first);
    expect(getDb().prepare("select count(*) n from learner_app_standard_credit_batches").get()).toMatchObject({ n: 1 });
  });

  it("does not create eight individual rows — one compact batch row (AC7)", async () => {
    await ensureMonthlyStandardAllocation(learnerId, appId, tz, new Date("2026-08-15T09:00:00.000Z"));
    const rows = getDb().prepare("select * from learner_app_standard_credit_batches").all();
    expect(rows).toHaveLength(1);
  });

  it("shows a fresh eight-credit balance with the ordinary two-session weekly limit", async () => {
    const allowance = await buildStandardAllowance(learnerId, appId, tz, new Date("2026-08-03T09:00:00.000Z"));
  ...
  it("funds two ordinary sessions per week and denies a third without catch-up eligibility", async () => {
    const monday = new Date("2026-08-03T09:00:00.000Z"); // 2026-W32
    const first = await fund(monday);
    expect(first.weeklySessionOrdinal).toBe(1);
    await consumeStandardReservation(first.batchId, learnerId, appId, first.weekKey, monday);
    const second = await fund(new Date("2026-08-04T09:00:00.000Z"));
    expect(second.weeklySessionOrdinal).toBe(2);
    await consumeStandardReservation(second.batchId, learnerId, appId, second.weekKey, new Date("2026-08-04T09:00:00.000Z"));
    await expect(fund(new Date("2026-08-05T09:00:00.000Z")))
      .rejects.toThrowError(new StandardCreditError("WEEKLY_STANDARD_SESSION_LIMIT_REACHED"));
  });

  it("allows a catch-up third session only when balance >8 and a batch expires this month (AC12-18)", async () => {
    ...
    await ensureMonthlyStandardAllocation(learnerId, appId, tz, new Date("2026-07-05T09:00:00.000Z"));
    const monday = new Date("2026-08-03T09:00:00.000Z");
    const first = await fund(monday); await consumeStandardReservation(first.batchId, learnerId, appId, first.weekKey, monday);
    const second = await fund(new Date("2026-08-04T09:00:00.000Z"));
    await consumeStandardReservation(second.batchId, learnerId, appId, second.weekKey, new Date("2026-08-04T09:00:00.000Z"));
    const allowance = await buildStandardAllowance(learnerId, appId, tz, new Date("2026-08-05T09:00:00.000Z"));
    expect(allowance).toMatchObject({ availableCount: 14, expiringThisMonthCount: 6, catchUpEligible: true, standardWeeklyLimit: 3 });
    const third = await fund(new Date("2026-08-05T09:00:00.000Z"));
  ...
  it("denies catch-up once the balance drops to exactly eight, and always denies a fourth session (AC15/AC19/AC20)", async () => {
    const monday = new Date("2026-08-03T09:00:00.000Z");
    const first = await fund(monday); await consumeStandardReservation(first.batchId, learnerId, appId, first.weekKey, monday);
    const second = await fund(new Date("2026-08-04T09:00:00.000Z"));
    await consumeStandardReservation(second.batchId, learnerId, appId, second.weekKey, new Date("2026-08-04T09:00:00.000Z"));
    ...
    await ensureMonthlyStandardAllocation(learnerId, appId, tz, new Date("2026-07-05T09:00:00.000Z"));
    const third = await fund(new Date("2026-08-05T09:00:00.000Z"));
    await consumeStandardReservation(third.batchId, learnerId, appId, third.weekKey, new Date("2026-08-05T09:00:00.000Z"));
    // a fourth is rejected outright, regardless of remaining balance
    await expect(fund(new Date("2026-08-06T09:00:00.000Z")))
      .rejects.toThrowError(new StandardCreditError("WEEKLY_STANDARD_SESSION_LIMIT_REACHED"));
  });

  it("reserves at start (no consumed_count change) and only consumes+counts the week at usable launch (AC41/AC42/AC43/AC44)", async () => {
    const now = new Date("2026-08-03T09:00:00.000Z");
    const funded = await fund(now);
    ...
    await consumeStandardReservation(funded.batchId, learnerId, appId, funded.weekKey, now);
    expect(getDb().prepare("select reserved_count,consumed_count from learner_app_standard_credit_batches where id=?").get(funded.batchId))
  ...
  it("releases an unexpired reservation back to available on pre-launch failure, but not an expired one (AC45/AC46)", async () => {
    const now = new Date("2026-08-03T09:00:00.000Z");
    const funded = await fund(now);
    expect(await releaseStandardReservation(funded.batchId, new Date("2026-08-03T09:05:00.000Z"))).toBe(true);
    ...
    const refunded = await fund(new Date("2026-08-03T09:10:00.000Z"));
    expect(await releaseStandardReservation(refunded.batchId, new Date("2026-09-30T18:30:00.001Z"))).toBe(false);
  });

  it("denies funding when no standard credit is available", async () => {
    const db = getDb();
    const batch = await ensureMonthlyStandardAllocation(learnerId, appId, tz, new Date("2026-08-03T09:00:00.000Z"));
    db.prepare("update learner_app_standard_credit_batches set consumed_count=8 where id=?").run(batch.id);
    await expect(fund(new Date("2026-08-03T09:00:00.000Z")))
      .rejects.toThrowError(new StandardCreditError("STANDARD_SESSION_CREDIT_UNAVAILABLE"));
  });
});
...
describe("EN-001 entitlement-period-keyed standard credit batches", () => {
  it("allocates 8 credits keyed by entitlement_period_id, not allocation_month, and is idempotent", async () => {
    const periodId = "period-1";
    seedEntitlementPeriod(periodId);
    const first = await ensureEntitlementPeriodStandardAllocation(learnerId, appId, periodId,
    ...
    const second = await ensureEntitlementPeriodStandardAllocation(learnerId, appId, periodId,
  ...
  it("reserve/consume/release work unchanged against a period-keyed batch id", async () => {
    seedEntitlementPeriod("period-2");
    const batch = await ensureEntitlementPeriodStandardAllocation(learnerId, appId, "period-2",
      "2026-08-10T00:00:00.000Z", "2026-10-11T00:00:00.000Z", new Date("2026-08-10T00:05:00.000Z"));
    const db = getDb();
    db.prepare("update learner_app_standard_credit_batches set reserved_count=1 where id=?").run(batch.id);
    await consumeStandardReservation(batch.id, learnerId, appId, "2026-W33", new Date("2026-08-10T00:10:00.000Z"));
    ...
    db.prepare("update learner_app_standard_credit_batches set reserved_count=1 where id=?").run(batch.id);
    expect(await releaseStandardReservation(batch.id, new Date("2026-08-10T00:15:00.000Z"))).toBe(true);
