...
describe("AD-005 getGovernanceOverview (AT-AD-005-05/06, rules 16-17, 96-99)", () => {
  it("counts staff by status, including the auto-seeded bootstrap Platform Administrator", async () => {
  ...
  it("flags staff without an active passkey — every seeded test staff account has none", async () => {
    seedStaffSession(["support_agent"]);
    const overview = await getGovernanceOverview();
    expect(overview.securityAlerts.staffWithoutActivePasskey).toBeGreaterThanOrEqual(1);
  });

  it("rule 29/99: last-Platform-Administrator risk is informational only, never a bypass of the real rule elsewhere", async () => {
    const overview = await getGovernanceOverview();
  ...
  it("recoveryCodesLow flags once active codes drop below two", async () => {
    const before = await getGovernanceOverview();
    ...
    const after = await getGovernanceOverview();
    expect(after.recoveryCodeStatus.activeCount).toBe(1);
    expect(after.securityAlerts.recoveryCodesLow).toBe(true);
  });

  it("recentPrivilegedActionCount is derived from the same three sources the audit viewer reads, never a fourth table", async () => {
    const overview = await getGovernanceOverview();
