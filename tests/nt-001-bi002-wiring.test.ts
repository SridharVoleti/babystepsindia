...
async function checkout(options: { autoRenewEnabled?: boolean; key?: string } = {}) {
...
beforeEach(async () => {
  ...
  productId = (await defineProductVersion({ id: "product-nt001-bi002", slug: "nt001-bi002-monthly", name: "Math Monthly",
...
describe("NT-001 real wiring: BI-002 (AT-NT-001-14/15/16)", () => {
  it("AT-NT-001-15: a failed renewal that enters grace enqueues exactly one billing_grace_started notification", async () => {
  ...
  it("a replayed failed-renewal provider event does not create a second grace-started notification", async () => {
    const active = await activate();
    ...
    await processVerifiedPaymentEvent(event, now);
  ...
  it("AT-NT-001-16: a payment recovered from grace enqueues exactly one billing_payment_recovered notification", async () => {
    const active = await activate();
    await processVerifiedPaymentEvent(renewalEvent(active.subscriptionId, "failed",
      { eventType: "renewal_payment_failed" }), new Date("2026-09-10T10:05:00.000Z"));
    await processVerifiedPaymentEvent(renewalEvent(active.subscriptionId, "recovered",
  ...
  it("AT-NT-001-14: the T-7 sweep enqueues a billing_renewal_reminder alongside the legacy notifier", async () => {
    const active = await activate();
  ...
  it("delivery failure never rolls back the already-committed billing transaction (AT-NT-001-27)", async () => {
    const active = await activate();
