...
describe("NT-002 composeParentCommunicationHistory", () => {
  ...
  it("AT-NT-002-06: a replayed provider webhook callback never duplicates the row", async () => {
    ...
    await ingestNotificationProviderEvent({ provider: "test", providerEventId: `wh-${randomUUID()}`, timestampSeconds, signatureHex, rawBody, secret: SECRET, now });
  ...
  it("AT-NT-002-08: deterministic keyset pagination — stable pages, no duplicates or gaps", async () => {
    ...
    const page1 = await composeParentCommunicationHistory(parentId, { limit: "2" }, new Date("2026-08-13T00:00:00.000Z"));
    expect(page1.items).toHaveLength(2);
    expect(page1.nextCursor).toBeTruthy();
    const page2 = await composeParentCommunicationHistory(parentId, { limit: "2", cursor: page1.nextCursor ?? undefined }, new Date("2026-08-13T00:00:00.000Z"));
    expect(page2.items).toHaveLength(2);
    expect(page2.nextCursor).toBeTruthy();
    const page3 = await composeParentCommunicationHistory(parentId, { limit: "2", cursor: page2.nextCursor ?? undefined }, new Date("2026-08-13T00:00:00.000Z"));
  ...
  it("AT-NT-002-09: retention boundary — items older than 13 months are excluded", async () => {
...
describe("NT-002 (NT2-G01) learner filtering applied before pagination", () => {
  async function makeLearner(displayName: string) {
    return (await createLearner(parentId, { displayName, dateOfBirth: "2018-01-01", idempotencyKey: randomUUID() },
      "2026-08-13")).learner;
  }
  ...
  it("filtered pagination produces no gaps or duplicates across a full learner-scoped dataset", async () => {
    const learnerA = await makeLearner("Learner A");
    const learnerB = await makeLearner("Learner B");
    const aIds: string[] = [];
    for (let i = 0; i < 7; i++) {
    ...
    for (let page = 0; page < 10; page++) {
      const result = await composeParentCommunicationHistory(
  ...
  it("structured learnerId matches regardless of a later learner rename; legacy display-name-only rows do not", async () => {
    ...
    const afterRename = await composeParentCommunicationHistory(
