...
describe("LP-001 transactional learner creation", () => {
  it("derives ownership, creates no billing data, and completes first-learner onboarding", async () => {
    ...
    const result = await createLearner(user.id, {
      displayName: " Asha ",
    ...
    expect(await listOwnedLearners(user.id, "2026-08-04")).toHaveLength(1);
  ...
  it("returns the original learner for the same idempotent request", async () => {
    ...
    const first = await createLearner(user.id, input, "2026-08-04");
    const retry = await createLearner(user.id, input, "2026-08-05");

    expect(retry.learner.id).toBe(first.learner.id);
    expect(await listOwnedLearners(user.id, "2026-08-05")).toHaveLength(1);
  });
  ...
  it("rejects conflicting idempotency reuse and normalized duplicates per parent", async () => {
    ...
    await expect(createLearner(user.id, {
      displayName: "Different",
      dateOfBirth: "2018-04-03",
      idempotencyKey: "33333333-3333-4333-8333-333333333333",
    }, "2026-08-04")).rejects.toThrowError(new LearnerCreationError("IDEMPOTENCY_KEY_REUSED"));
  ...
  it("allows the same normalized name for a different parent", async () => {
    ...
    await createLearner(firstParent.id, { displayName: "Aarav", dateOfBirth: "2018-01-01", idempotencyKey: "55555555-5555-4555-8555-555555555555" }, "2026-08-04");
    await createLearner(secondParent.id, { displayName: "aarav", dateOfBirth: "1980-01-01", idempotencyKey: "66666666-6666-4666-8666-666666666666" }, "2026-08-04");
  });
});
