...
describe("NT-003 composeParentNotificationPreferences", () => {
  it("AT-NT-003-01/33: a new parent gets one canonical policy view, defaulting learning reminders on", async () => {
    const prefs = await composeParentNotificationPreferences(parentId, new Date("2026-08-15T00:00:00.000Z"));
  ...
  it("AT-NT-003-03/04/05/06: all four mandatory categories are present and Always on", async () => {
    const prefs = await composeParentNotificationPreferences(parentId, new Date());
  ...
  it("AT-NT-003-08/09: exactly one editable preference exists, no global switch", async () => {
    const prefs = await composeParentNotificationPreferences(parentId, new Date());
  ...
  it("AT-NT-003-30: a verified parent gets a masked current-email hint, never the raw address", async () => {
  ...
  it("AT-NT-003-31: an unverified/never-verified parent gets no email hint, never a guess", async () => {
    const prefs = await composeParentNotificationPreferences(parentId, new Date());
    expect(prefs.destination.verifiedEmailHint).toBeUndefined();
  });

  it("AT-NT-003-43/46: exposes the NT-002 communication-history route as a safe secondary link", async () => {
    const prefs = await composeParentNotificationPreferences(parentId, new Date());
    expect(prefs.communicationHistoryRoute).toBe(COMMUNICATION_HISTORY_ROUTE);
  });

  it("AT-NT-003-49/50: never exposes provider/source/delivery internals", async () => {
    const prefs = await composeParentNotificationPreferences(parentId, new Date());
...
describe("NT-003 applyParentNotificationPreferenceChange", () => {
  it("AT-NT-003-12/13: toggles the shared EG-006 field and bumps the version", async () => {
    const before = await composeParentNotificationPreferences(parentId, new Date());
    const after = await applyParentNotificationPreferenceChange(parentId,
  ...
  it("AT-NT-003-37: a stale expectedVersion is rejected, not silently applied", async () => {
    await expect(applyParentNotificationPreferenceChange(parentId,
      { learningReminderEmailEnabled: false, expectedVersion: 99, idempotencyKey: `k-${randomUUID()}` }, new Date())).rejects.toThrow(LearningReminderError);
  });

  it("AT-NT-003-38: the same idempotency key with the same payload replays the prior result", async () => {
    const key = `k-${randomUUID()}`;
    const first = await applyParentNotificationPreferenceChange(parentId,
      { learningReminderEmailEnabled: false, expectedVersion: 1, idempotencyKey: key }, new Date());
    const second = await applyParentNotificationPreferenceChange(parentId,
      { learningReminderEmailEnabled: false, expectedVersion: 1, idempotencyKey: key }, new Date());
    expect(second.preferenceVersion).toBe(first.preferenceVersion);
  });

  it("AT-NT-003-39: the same idempotency key with a conflicting payload is rejected", async () => {
    const key = `k-${randomUUID()}`;
    await applyParentNotificationPreferenceChange(parentId,
      { learningReminderEmailEnabled: false, expectedVersion: 1, idempotencyKey: key }, new Date());
    await expect(applyParentNotificationPreferenceChange(parentId,
      { learningReminderEmailEnabled: true, expectedVersion: 1, idempotencyKey: key }, new Date())).rejects.toThrow(LearningReminderError);
  });

  it("AT-NT-003-14: the preference change is parent-wide — no learnerId/appId is accepted or needed", async () => {
    const before = await composeParentNotificationPreferences(parentId, new Date());
    const result = await applyParentNotificationPreferenceChange(parentId,
  ...
  it("AT-NT-003-27: the write path has no field for mandatory categories at all — nothing to tamper", async () => {
