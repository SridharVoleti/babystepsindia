...
describe("composeParentShellContext — AT-PD-004-01/07/08/28/47 (API-PD-006)", () => {
  it("AT-PD-004-07: wraps PD-003's exact badge as attentionSummary — the only attention algorithm, not a second one", () => {
    const badge = attentionService.composeParentAttentionBadge(parentId, now);
    const context = composeParentShellContext(parentId, 0, now);
  ...
  it("is a pure read — writes nothing to the database", () => {
    ...
    composeParentShellContext(parentId, 0, now);
    const after = (getDb().prepare("select count(*) as n from learners").get() as { n: number }).n;
    expect(after).toBe(before);
  });

  it("AT-PD-004-28: passes the caller-derived modeGeneration through unchanged and reflects it in a distinct shellVersion", () => {
    const at0 = composeParentShellContext(parentId, 0, now);
    const at1 = composeParentShellContext(parentId, 1, now);
    expect(at0.modeGeneration).toBe(0);
    expect(at1.modeGeneration).toBe(1);
    expect(at0.shellVersion).not.toBe(at1.shellVersion);
  });

  it("AT-PD-004-10/G03: navItems come from the one canonical PARENT_NAV_ITEMS source", () => {
    const context = composeParentShellContext(parentId, 0, now);
    expect(context.navItems).toEqual(PARENT_NAV_ITEMS);
  });

  it("AT-PD-004-08: attentionSummary failure degrades to undefined without throwing — nav/mode still returned", () => {
    vi.spyOn(attentionService, "composeParentAttentionBadge").mockImplementation(() => { throw new Error("boom"); });
    const context = composeParentShellContext(parentId, 2, now);
  ...
  it("AT-PD-004-47: capabilityHints are present but non-authoritative — no new auth table involved", () => {
    const context = composeParentShellContext(parentId, 0, now);
