...
describe("PR-004 runReconciliationSweep", () => {
  it("processes every row across pages via cursor round-trip, in bounded order", async () => {
    ...
    const first = runReconciliationSweep({ environment, limit: 2, runIdempotencyKey: "run-1", now });
    expect(first.processed).toBe(2);
    expect(first.nextCursor).not.toBeNull();

    const second = runReconciliationSweep({ environment, cursor: first.nextCursor!, limit: 2,
  ...
  it("opens an incident for a hash-mismatched row discovered during the sweep", async () => {
    ...
    const result = runReconciliationSweep({ environment, limit: 10, runIdempotencyKey: "run-2", now });
  ...
  it("counts a repair when a blocked_repairable_metadata row is found already self-corrected", async () => {
    ...
    const first = runReconciliationSweep({ environment, limit: 10, runIdempotencyKey: "run-3a", now });
    ...
    const second = runReconciliationSweep({ environment, limit: 10, runIdempotencyKey: "run-3b", now });
  ...
  it("is idempotent: an identical runIdempotencyKey+cursor returns the cached result without reprocessing", async () => {
    await learnerWithProgress({ badHash: true, updatedAt: "2026-08-01T00:00:00.000Z" });
    const first = runReconciliationSweep({ environment, limit: 10, runIdempotencyKey: "run-4", now });
    const countAfterFirst = (getDb().prepare(`select count(*) as n from progress_integrity_incidents`).get() as { n: number }).n;
    const second = runReconciliationSweep({ environment, limit: 10, runIdempotencyKey: "run-4", now });
  ...
  it("scopes to a single app when appId is supplied", async () => {
    ...
    const result = runReconciliationSweep({ appId, environment, limit: 10, runIdempotencyKey: "run-5", now });
    expect(result.processed).toBe(1);
  });
});
