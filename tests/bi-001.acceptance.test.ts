...
async function createProduct(overrides: Partial<Parameters<typeof defineProductVersion>[0]> = {}) {
  return await defineProductVersion({ id: "product-math", slug: "math-monthly", name: "Math Monthly",
...
async function activate(learnerId = sourceLearnerId, suffix = "1", product = productId) {
  const checkout = await createCheckoutIntent(parentId, { learnerId, productId: product, productVersion: 1,
    idempotencyKey: `checkout-${suffix}` }, { now: NOW, provider });
  return await applyVerifiedCheckoutPayment({ provider: checkout.provider,
...
async function caseFor(subscriptionId: string, suffix = "1") {
  return await createReassignmentCase(parentId, { subscriptionId, targetLearnerId,
    reasonCode: "WRONG_LEARNER_SELECTED", notes: "Please correct the assignment.",
    idempotencyKey: `case-${suffix}` }, NOW);
}

async function execute(subscriptionId: string, caseId: string, mode: "immediate_if_unused" | "next_period",
  key = "admin-request-1") {
  ...
  return await executeSubscriptionReassignment("admin-1", subscriptionId, { caseId, targetLearnerId,
...
describe("BI-001 purchase and immutable learner assignment", () => {
  it("AT-BI-001-01..04 derives purchaser from auth, binds one owned learner and returns explicit confirmation", async () => {
    const checkout = await createCheckoutIntent(parentId, { learnerId: sourceLearnerId, productId, productVersion: 1,
  ...
  it("AT-BI-001-03 denies a foreign learner without creating an intent", async () => {
    ...
    await expect(createCheckoutIntent(parentId, { learnerId: foreign, productId, productVersion: 1,
      idempotencyKey: "foreign" }, { now: NOW, provider })).rejects.toThrow(new BillingAssignmentError("RESOURCE_NOT_FOUND"));
    expect((getDb().prepare("select count(*) n from checkout_intents").get() as { n: number }).n).toBe(0);
  });

  it("AT-BI-001-05..07 supports one-app and bundle snapshots without learner arrays", async () => {
    const bundle = await createProduct({ id: "bundle-1", slug: "bundle", name: "All Apps", planReference: "plan_bundle",
      productType: "bundle", appIds: [APP_MATH, APP_READING] });
    const activated = await activate(sourceLearnerId, "bundle", bundle.id);
  ...
  it("AT-BI-001-08 exact checkout retry returns original; a conflicting retry is rejected", async () => {
    const input = { learnerId: sourceLearnerId, productId, productVersion: 1, idempotencyKey: "retry" };
    const first = await createCheckoutIntent(parentId, input, { now: NOW, provider });
    expect(await createCheckoutIntent(parentId, input, { now: NOW, provider })).toEqual(first);
    await expect(createCheckoutIntent(parentId, { ...input, learnerId: targetLearnerId }, { now: NOW, provider })).rejects.toThrow(new BillingAssignmentError("IDEMPOTENCY_KEY_REUSED"));
  });

  it("AT-BI-001-09 failed payment creates no active subscription", async () => {
    const checkout = await createCheckoutIntent(parentId, { learnerId: sourceLearnerId, productId, productVersion: 1,
      idempotencyKey: "failed" }, { now: NOW, provider });
    expect(await applyVerifiedCheckoutPayment({ provider: checkout.provider, providerEventId: "failed-event",
      ...
      paymentStatus: "failed", providerSubscriptionRef: "none", periodStart: NOW.toISOString(), periodEnd: PERIOD_END }, NOW)).toBeNull();
    expect((getDb().prepare("select count(*) n from subscriptions").get() as { n: number }).n).toBe(0);
  });

  it("AT-BI-001-10 rejects a provider context mismatch atomically", async () => {
    const checkout = await createCheckoutIntent(parentId, { learnerId: sourceLearnerId, productId, productVersion: 1,
      idempotencyKey: "mismatch" }, { now: NOW, provider });
    await expect(applyVerifiedCheckoutPayment({ provider: checkout.provider, providerEventId: "mismatch-event",
      checkoutIntentId: checkout.checkoutIntentId, providerCheckoutRef: checkout.providerCheckoutRef,
      purchaserParentId: parentId, assignedLearnerId: targetLearnerId, productId, productVersion: 1,
      paymentStatus: "paid", providerSubscriptionRef: "provider-sub", periodStart: NOW.toISOString(), periodEnd: PERIOD_END }, NOW)).rejects.toThrow(new BillingAssignmentError("CHECKOUT_CONTEXT_MISMATCH"));
    expect((getDb().prepare("select count(*) n from subscriptions").get() as { n: number }).n).toBe(0);
  });
  ...
  it("AT-BI-001-11 has no self-service assignment mutation and AT-BI-001-12 learner rename keeps the ID", async () => {
    ...
    const summary = (await listParentSubscriptions(parentId)).items[0];
...
describe("BI-001 case-based administrator reassignment", () => {
  it("AT-BI-001-13 creates an open parent case without changing assignment", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
  ...
  it("AT-BI-001-16 rejects a cross-parent target non-enumerating", async () => {
    ...
    await expect(createReassignmentCase(parentId, { subscriptionId: active.subscriptionId,
      targetLearnerId: foreign, reasonCode: "WRONG_LEARNER_SELECTED", idempotencyKey: "foreign-case" }, NOW)).rejects.toThrow(new BillingAssignmentError("RESOURCE_NOT_FOUND"));
  });

  it("AT-BI-001-17 denies permanently ended/refunded/disputed states", async () => {
    for (const status of ["expired", "refunded", "disputed"] as const) {
      const active = await activate(sourceLearnerId, status);
      const created = await caseFor(active.subscriptionId, status);
      getDb().prepare("update subscriptions set status=? where id=?").run(status, active.subscriptionId);
      await expect(execute(active.subscriptionId, created.caseId, "immediate_if_unused", `admin-${status}`)).rejects.toThrow(new BillingAssignmentError("SUBSCRIPTION_REASSIGNMENT_NOT_ALLOWED"));
    }
  });

  it("AT-BI-001-18 rejects target product/app overlap", async () => {
    const sourceSubscription = await activate(sourceLearnerId, "source");
    await activate(targetLearnerId, "target");
    const created = await caseFor(sourceSubscription.subscriptionId, "overlap");
    await expect(execute(sourceSubscription.subscriptionId, created.caseId, "immediate_if_unused")).rejects.toThrow(new BillingAssignmentError("TARGET_SUBSCRIPTION_CONFLICT"));
  });
  ...
  it.each([["source", () => sourceLearnerId], ["target", () => targetLearnerId]] as const)(
    ...
    async (_label, learnerFactory) => {
      const active = await activate();
      const created = await caseFor(active.subscriptionId);
      seedSession(learnerFactory(), "active");
      await expect(execute(active.subscriptionId, created.caseId, "immediate_if_unused")).rejects.toThrow(new BillingAssignmentError("LEARNER_SESSION_IN_PROGRESS"));
    },
  );

  it("AT-BI-001-21 immediate unused correction updates assignment and emits one event", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
    const result = await execute(active.subscriptionId, created.caseId, "immediate_if_unused");
  ...
  it("AT-BI-001-22 used subscription cannot be corrected immediately", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
    seedSession(sourceLearnerId, "completed", true);
    await expect(execute(active.subscriptionId, created.caseId, "immediate_if_unused")).rejects.toThrow(new BillingAssignmentError("REASSIGNMENT_SCHEDULE_REQUIRED"));
  });
  ...
  it("AT-BI-001-23..26 schedules a used subscription and switches atomically at the boundary without moving history", async () => {
    ...
    const scheduled = await execute(active.subscriptionId, created.caseId, "next_period");
  ...
  it("AT-BI-001-27 purchaser is structurally immutable", async () => {
    const active = await activate();
  ...
  it("AT-BI-001-28 closed/rejected/executed cases cannot execute", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
    getDb().prepare("update subscription_reassignment_cases set status='closed' where id=?").run(created.caseId);
    await expect(execute(active.subscriptionId, created.caseId, "immediate_if_unused")).rejects.toThrow(new BillingAssignmentError("CASE_NOT_ACTIVE"));
  });

  it("AT-BI-001-29/30 exact admin retry returns original; conflicting reuse is rejected", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
    const first = await execute(active.subscriptionId, created.caseId, "immediate_if_unused", "admin-retry");
    const retry = await executeSubscriptionReassignment("admin-1", active.subscriptionId, { caseId: created.caseId,
    ...
    await expect(executeSubscriptionReassignment("admin-1", active.subscriptionId, { caseId: created.caseId,
      targetLearnerId, effectiveMode: "next_period", reasonCode: "WRONG_LEARNER_SELECTED",
      expectedSubscriptionVersion: 1, expectedCaseVersion: 1, idempotencyKey: "admin-retry" }, NOW)).rejects.toThrow(new BillingAssignmentError("IDEMPOTENCY_KEY_REUSED"));
  });

  it("AT-BI-001-31 stale subscription or case versions cause no partial change", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
    await expect(executeSubscriptionReassignment("admin-1", active.subscriptionId, { caseId: created.caseId,
      targetLearnerId, effectiveMode: "immediate_if_unused", reasonCode: "WRONG_LEARNER_SELECTED",
      expectedSubscriptionVersion: 99, expectedCaseVersion: 1, idempotencyKey: "stale" }, NOW)).rejects.toThrow(new BillingAssignmentError("VERSION_CONFLICT"));
    expect((getDb().prepare("select assigned_learner_id from subscriptions where id=?").get(active.subscriptionId) as any)
      .assigned_learner_id).toBe(sourceLearnerId);
  });

  it("AT-BI-001-32 outbox failure rolls back assignment and case state", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
    ...
    await expect(execute(active.subscriptionId, created.caseId, "immediate_if_unused")).rejects.toThrow(/outbox failed/);
    expect((getDb().prepare("select assigned_learner_id from subscriptions where id=?").get(active.subscriptionId) as any)
      .assigned_learner_id).toBe(sourceLearnerId);
  ...
  it("AT-BI-001-33..35 keeps app/payment/log boundaries narrow and assignment device-independent", async () => {
    const active = await activate();
    const created = await caseFor(active.subscriptionId);
    const adminView = await getAdminReassignmentCase(created.caseId);
