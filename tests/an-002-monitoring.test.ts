...
describe("AN-002 syncMonitoringSnapshots / getOperationalStatus", () => {
  it("AC1: exposes last run/status for a critical job from its own source table", async () => {
    seedBillingJobRun("completed", "2026-08-10T00:00:00.000Z", "2026-08-10T00:01:00.000Z");
    await syncMonitoringSnapshots(new Date("2026-08-10T00:02:00.000Z"));
    const status = (await getOperationalStatus()).find((j) => j.jobKey === "billing_reconcile");
    expect(status?.status).toBe("completed");
    expect(status?.lastRunAt).toBe("2026-08-10T00:00:00.000Z");
    expect(status?.durationMs).toBe(60_000);
  });

  it("AC2: dependency failures are distinguishable from successes", async () => {
    seedBillingJobRun("failed", "2026-08-10T00:00:00.000Z", "2026-08-10T00:01:00.000Z");
    await syncMonitoringSnapshots(new Date("2026-08-10T00:02:00.000Z"));
    expect((await getOperationalStatus()).find((j) => j.jobKey === "billing_reconcile")?.status).toBe("failed");
  });

  it("a job with no runs yet reports 'unknown', never a fabricated status", async () => {
    expect((await getOperationalStatus()).find((j) => j.jobKey === "billing_renewal_reminder")?.status).toBe("unknown");
  });

  it("AC4/rule: syncing never mutates the source table it reads from", async () => {
    seedBillingJobRun("completed", "2026-08-10T00:00:00.000Z", "2026-08-10T00:01:00.000Z");
  ...
  it("is idempotent — re-syncing the same source rows never duplicates a snapshot", async () => {
    seedBillingJobRun("completed", "2026-08-10T00:00:00.000Z", "2026-08-10T00:01:00.000Z");
    await syncMonitoringSnapshots(new Date("2026-08-10T00:02:00.000Z"));
    await syncMonitoringSnapshots(new Date("2026-08-10T00:03:00.000Z"));
  ...
  it("carries safe, non-payload counts through for the progress-integrity sweep source", async () => {
    seedProgressIntegritySweepRun("2026-08-10T00:00:00.000Z");
    await syncMonitoringSnapshots(new Date("2026-08-10T00:02:00.000Z"));
    const status = (await getOperationalStatus()).find((j) => j.jobKey === "progress_integrity_sweep");
...
describe("AN-002 compactMonitoringHistory (AC3: 30-day detail + 12-month aggregate retention)", () => {
  it("rolls a detail snapshot older than 30 days into a monthly aggregate, then purges the detail row", async () => {
    const old = new Date("2026-06-01T00:00:00.000Z");
    seedBillingJobRun("completed", old.toISOString(), old.toISOString());
    await syncMonitoringSnapshots(old);
    ...
    const aggregates = await listMonthlyAggregates("billing_reconcile");
    expect(aggregates).toContainEqual(expect.objectContaining({ month_key: "2026-06", run_count: 1, failed_count: 0 }));
  });

  it("keeps a detail snapshot within the last 30 days untouched", async () => {
    const recent = new Date(Date.now() - 5 * 24 * 60 * 60_000).toISOString();
    seedBillingJobRun("completed", recent, recent);
    await syncMonitoringSnapshots(new Date(recent));
    await compactMonitoringHistory(new Date());
    const remaining = getDb().prepare("select count(*) n from monitoring_job_snapshots").get() as { n: number };
    expect(remaining.n).toBe(1);
  });

  it("never touches the source job-run table during compaction either", async () => {
    const old = new Date("2026-06-01T00:00:00.000Z");
    seedBillingJobRun("completed", old.toISOString(), old.toISOString());
    await syncMonitoringSnapshots(old);
    const before = getDb().prepare("select count(*) n from billing_job_runs").get();
    await compactMonitoringHistory(new Date("2026-08-15T00:00:00.000Z"));
    expect(getDb().prepare("select count(*) n from billing_job_runs").get()).toEqual(before);
  });

  it("purges a monthly aggregate older than 12 months", async () => {
    getDb().prepare(
    ...
    await compactMonitoringHistory(new Date("2026-08-15T00:00:00.000Z"));
    expect(await listMonthlyAggregates("billing_reconcile")).toEqual([]);
  });
});
