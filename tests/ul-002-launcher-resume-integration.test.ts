...
describe("UL-002 launcher and original-device resume", () => {
  it("shows Resume only for the resumable app and blocks every other Start", async () => {
    ...
    const resumed = await resumeLearnerSession({ grantId: "grant", principalId: "principal", learnerSessionId: sessionId,
  ...
  it("rejects resume from another device without changing the durable resumable state", async () => {
    ...
    await expect(resumeLearnerSession({ grantId: "grant", principalId: "principal", learnerSessionId: sessionId,
      learnerId, appId }, { deviceSessionId: "other-device", credential, now })).rejects.toThrow("SESSION_RESUME_DEVICE_MISMATCH");
