...
describe("BR-002 startRecoveryTestRecord", () => {
  it("starts a new evidence record with the initiating staff account and outbound-suppression flag", () => {
    const record = startRecoveryTestRecord(
  ...
  it("rejects an empty backup or temp-project reference", () => {
    expect(() => startRecoveryTestRecord(
      { staffAccountId: ADMIN },
      { backupReference: "  ", tempProjectReference: "temp-1", outboundProcessingSuppressed: true, idempotencyKey: "k-2" },
    )).toThrow(DisasterRecoveryError);
  });

  it("replaying the same idempotencyKey returns the same record rather than creating a second one", () => {
    const first = startRecoveryTestRecord(
      { staffAccountId: ADMIN },
      { backupReference: "b-1", tempProjectReference: "t-1", outboundProcessingSuppressed: true, idempotencyKey: "k-3" },
    );
    const second = startRecoveryTestRecord(
      { staffAccountId: ADMIN },
      { backupReference: "b-1", tempProjectReference: "t-1", outboundProcessingSuppressed: true, idempotencyKey: "k-3" },
    );
...
describe("BR-002 updateRecoveryTestRecord", () => {
  ...
  it("records a single step's confirmation and notes without touching the others", () => {
    const record = startRecord("u-1");
    const updated = updateRecoveryTestRecord(
      { staffAccountId: ADMIN },
  ...
  it("sets completedAt only once all 4 validation steps are confirmed", () => {
    const record = startRecord("u-2");
    updateRecoveryTestRecord({ staffAccountId: ADMIN }, { recordId: record.id, deletionReplay: { confirmed: true }, idempotencyKey: "u-2-a" });
    updateRecoveryTestRecord({ staffAccountId: ADMIN }, { recordId: record.id, billingReconciliation: { confirmed: true }, idempotencyKey: "u-2-b" });
    updateRecoveryTestRecord({ staffAccountId: ADMIN }, { recordId: record.id, derivableStateRebuild: { confirmed: true }, idempotencyKey: "u-2-c" });
    let latest = getRecoveryTestRecord(record.id)!;
    expect(latest.completedAt).toBeNull();
    const finalUpdate = updateRecoveryTestRecord(
  ...
  it("teardown is recorded independently of validation completion", () => {
    const record = startRecord("u-3");
    const updated = updateRecoveryTestRecord(
  ...
  it("throws RECORD_NOT_FOUND for an unknown record id", () => {
    expect(() => updateRecoveryTestRecord(
      { staffAccountId: ADMIN }, { recordId: "does-not-exist", teardownConfirmed: true, idempotencyKey: "u-4" },
    )).toThrow(DisasterRecoveryError);
  });

  it("never mutates a source-of-truth table — only its own record", () => {
    const record = startRecord("u-5");
    const before = getDb().prepare("select count(*) n from staff_accounts").get();
    updateRecoveryTestRecord({ staffAccountId: ADMIN }, { recordId: record.id, deletionReplay: { confirmed: true }, idempotencyKey: "u-5-step" });
